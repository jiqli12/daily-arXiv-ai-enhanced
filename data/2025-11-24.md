...
TL;DR: 使用持久同调（PH）方法从拓扑数据分析角度客观区分艺术风格，能够统计显著地区分不同艺术家、同一流派内的艺术家，以及区分艺术家作品与AI生成的艺术风格图像。
...
Result: PH方法能够以统计显著性区分不同艺术家（包括不同艺术流派和同一流派内），并能有效区分艺术家原作与AI生成的风格模仿图像。

Conclusion: 持久同调为艺术风格分析提供了客观、可解释的洞察力，是传统主观艺术分析方法的有效补充。
...
TL;DR: 本文提出了首个专门为双人肖像生成设计的大规模基准数据集PairHuman，包含超过10万张图像，并开发了DHumanDiff基线方法，在双人肖像生成中实现了增强的面部一致性和个性化生成与场景创建的平衡。
...
Method: 构建PairHuman数据集（包含10万+图像及丰富元数据），并提出DHumanDiff基线方法，该方法增强面部一致性并平衡个性化人物生成与语义驱动场景创建。
...
TL;DR: SaFeR-CLIP通过将不安全概念重定向到语义上最接近的安全替代品，最小化表示变化，在保持安全性的同时恢复了高达8.0%的零样本准确率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）通过微调提高安全性时，往往导致泛化性能显著下降，这种权衡源于将不安全概念强制对齐到单一预定义安全目标的刚性策略。

Method: 提出邻近感知方法，将不安全概念重定向到语义上最接近的安全替代品，最小化表示变化，并引入SaFeR-CLIP微调框架应用最小干预原则。

Result: SaFeR-CLIP成功调和了安全性和性能，在先前方法基础上恢复了高达8.0%的零样本准确率，同时保持强大的安全性。还贡献了NSFW-Caps基准用于分布偏移下的安全性测试。
...
Method: 三阶段方法：1）将栅格化输入提升为3D表示并渲染多视图图像；2）扩展SAM2的时间记忆机制到空间域，建立部分级对应关系；3）在栅格到向量转换时进行路径整合和结构优化。
...
<details>
  <summary>Details</summary>
Motivation: 传统3D网格制作耗时且难以扩展，现有自回归模型存在质量与速度的权衡问题，且序列依赖限制了增量编辑能力。

Method: 受语言模型RAG启发，通过点云分割、空间变换和点云注册技术，检索、生成和集成网格组件，解耦生成过程的序列依赖。

Result: 在多种自回归网格生成模型上验证，显著提升网格质量，加速生成速度，并支持增量编辑，无需重新训练模型。

Conclusion: Mesh RAG框架有效解决了自回归网格生成的质量-速度权衡问题，实现了高效并行推理和增量编辑能力。
...
TL;DR: WorldGen是一个从文本提示自动创建大规模交互式3D世界的系统，结合了LLM场景布局推理、程序生成、基于扩散的3D生成和对象感知场景分解技术。
...
Method: 结合LLM驱动的场景布局推理、程序生成、基于扩散的3D生成和对象感知场景分解。

Result: 系统能够将自然语言描述转换为可遍历、完全纹理化的环境，这些环境可以立即在标准游戏引擎中探索或编辑。
...
<details>
  <summary>Details</summary>
Motivation: 现有地球观测数据集主要关注语义理解任务（如标题生成或分类），缺乏将多模态感知与可测量生物物理变量对齐的基准。

Method: 构建了一个认知可解释的逻辑链（人类活动、土地覆盖分类、生态斑块计数、地上生物量回归），整合了共配准的Sentinel-2和ALOS-2影像，通过混合人机流程生成和验证结构化文本标注。
...
TL;DR: 本文提出了BOP-ASK数据集，用于评估和训练视觉语言模型在物体交互推理方面的能力，包含超过150k图像和33M问答对，涵盖6个任务。


<details>
  <summary>Details</summary>
Motivation: 现有空间推理基准测试只关注高层次关系，忽略了真实世界应用所需的精细空间理解能力，如精确3D定位、物体物理兼容性、物体可供性和多步空间规划。

Method: 利用BOP数据集的6D物体姿态数据，生成精细标注（抓取姿态、参考物体姿态、路径规划轨迹、相对空间和深度关系、物体间关系），构建大规模数据集。

Result: 在BOP-ASK上训练的模型优于基线，展现出精确物体和抓取姿态估计、轨迹规划以及在杂乱环境中精细物体中心空间推理等新兴能力。

Conclusion: BOP-ASK为训练和评估VLMs提供了丰富资源，实验证明该数据集能显著提升模型在物体交互推理方面的性能，并释放了数据集和生成流程。
...
TL;DR: 提出了Parts-Mamba模型，一种混合GCN-Mamba架构，用于解决骨架动作识别中因遮挡或通信质量差导致骨架不完整的问题，通过捕捉远距离关节的上下文信息来提升识别性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中捕获的骨架常因人体部分遮挡或通信质量差而不完整，现有GCN模型在缺失局部上下文时表现不佳，需要增强捕捉和保持远距离关节上下文信息的能力。

Method: 提出Parts-Mamba混合模型，结合GCN和Mamba架构，通过部件特定扫描特征捕捉部件特定信息，并通过部件-身体融合模块保留非邻近关节的上下文。

Result: 在NTU RGB+D 60和NTU RGB+D 120数据集的不同遮挡设置下进行评估，准确率最高提升12.9%。

Conclusion: Parts-Mamba模型能有效处理骨架不完整问题，通过增强远距离上下文信息的捕捉能力显著提升骨架动作识别性能。
...
TL;DR: 本文提出了联合Gromov-Wasserstein (JGW) 目标，扩展了传统的GW距离框架，支持多对一和多对多对象匹配，提供了部分同构分布的非负差异度量，并在点云对象表示中实现了高效计算。


<details>
  <summary>Details</summary>
Motivation: 传统的Gromov-Wasserstein距离仅限于单对象之间的配对匹配，无法满足需要多对一或多对多对象匹配的应用场景需求。

Method: 通过引入JGW目标扩展GW框架，支持对象集合的同时匹配，采用最优传输算法（包括熵正则化）来求解点云对象表示。
...
TL;DR: 该论文提出了一种称为表示对齐（REPA）的方法，将扩散或流式生成模型与预训练的自监督视觉编码器（如DINOv2）进行内部表示对齐，以改进逆问题中的重建质量。


<details>
  <summary>Details</summary>
Motivation: 在逆问题中，预训练的生成模型被用作先验，但缺乏真实信号指导。作者希望利用表示对齐提供强大的归纳偏置，提高重建保真度和感知真实性。

Method: 提出REPA方法，在推理时对齐扩散或流式模型与预训练自监督编码器的表示，即使没有真实信号，也能通过近似目标特征对齐来指导重建过程。

Result: 理论分析显示REPA正则化与DINOv2嵌入空间中的散度度量相关，且REPA更新将模型内部表示导向干净图像。实验证明该方法在超分辨率、盒内修复、高斯去模糊和运动去模糊等任务中一致提高重建质量，同时通过减少离散化步骤提高效率。

Conclusion: REPA方法为逆问题提供了一种通用的表示对齐框架，能显著提升重建质量和效率，具有广泛的适用性。
...
### [14] [MultiPriv: Benchmarking Individual-Level Privacy Reasoning in Vision-Language Models](https://arxiv.org/abs/2511.16940)
*Xiongtao Sun,Hui Li,Jiaming Zhang,Yujie Yang,Kaili Liu,Ruxin Feng,Wen Jun Tan,Wei Yang Bryan Lim*

Main category: cs.CV

TL;DR: MultiPriv是首个系统评估视觉语言模型中个体级隐私推理风险的基准，提出了隐私感知与推理框架，构建了包含合成个体档案的双语多模态数据集，评估了50多个基础模型和商业模型，揭示了现有安全对齐对推理攻击无效的系统性漏洞。
...
Method: 提出隐私感知与推理框架，构建包含合成个体档案的双语多模态数据集，其中标识符与敏感属性精心链接。设计了9个挑战性任务，涵盖从属性检测到跨图像重识别和链式推理的完整PPR谱系。

Result: 大规模评估50多个模型发现：(1)许多模型存在显著但未测量的推理隐私风险；(2)感知级指标无法预测推理风险；(3)现有安全对齐对推理攻击不一致且无效。
...
<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，未能充分利用玻璃本身的固有特性。观察到现实场景中玻璃前后光照强度差异导致反射变化的现象，利用这一特性进行更准确的玻璃表面定位。
...
Conclusion: 利用闪光/无闪光图像中的反射动态特性是玻璃表面检测的有效方法，NFGlassNet通过专门的模块设计实现了优越的性能。
...
TL;DR: 本文提出了R-AVST数据集和AVST-Zero模型，用于解决真实世界音频-视觉时空推理任务。R-AVST包含5K个未修剪视频和27K个对象，覆盖100种音频-视觉事件；AVST-Zero采用强化学习方法，无需中间监督，直接优化行为。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频理解方面进展迅速，但主要关注简单视频场景，无法反映真实世界音频-视觉事件的复杂性和多样性。

Method: 构建R-AVST数据集：使用基于LLM的关键对象提取、自动空间标注和人工质量检查的流程；提出AVST-Zero模型：基于强化学习，避免中间监督，通过精心设计的多维度奖励直接优化行为。

Result: R-AVST数据集包含5K个未修剪视频、27K个对象和8K多个高质量问答对；AVST-Zero在R-AVST上表现出与现有模型竞争的性能。
...
TL;DR: 本文提出了Warm Diffusion，一种统一的模糊-噪声混合扩散模型（BNMD），通过联合控制模糊和噪声来整合热扩散和冷扩散的优势。


<details>
  <summary>Details</summary>
Motivation: 热扩散完全依赖噪声，未能利用图像高频细节与低频结构之间的强相关性；冷扩散仅使用模糊而忽略噪声在塑造数据流形中的作用，导致性能下降。

Method: 采用分治策略，利用图像中的频谱依赖性，通过解耦去噪和去模糊过程来简化分数模型估计，并分析模糊-噪声比（BNR）来研究模型学习动态与数据流形变化之间的权衡。
...
TL;DR: Q-Real是一个用于AI生成图像细粒度评估的数据集，专注于真实性和合理性两个关键维度，包含3,088张图像及其标注，并构建了Q-Real Bench基准来评估多模态大语言模型在判断和推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有质量评估数据集和方法通常只提供单一质量分数，过于粗糙，无法为改进生成模型提供针对性指导。随着统一生成-理解模型的出现，细粒度评估对提升生成性能尤为重要。

Method: 构建Q-Real数据集，包含3,088张由流行文本到图像模型生成的图像，标注主要实体的位置，并提供真实性和合理性维度的判断问题和归因描述。设计微调框架并在多个MLLMs上进行实验。

Result: 实验结果表明数据集质量高且具有重要意义，基准评估全面。数据集和代码将在发表后发布。

Conclusion: Q-Real数据集和基准为AI生成图像的细粒度评估提供了有效工具，能够促进多模态大语言模型能力的提升，并为改进生成模型提供针对性指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有的异常生成方法通常需要少量异常样本进行微调，这与异常稀缺的实际情况相矛盾，且容易过拟合类别先验。本文旨在解决无真实异常样本或训练可用的场景。

Method: 提出Delta-Denoising方法：1）通过最小提示对驱动两个扩散分支对比；2）累积每步去噪差异生成图像特定定位图；3）使用该掩码指导潜在修复，在保留上下文的同时生成真实局部缺陷；4）进行标记级提示优化和空间注意力偏置。

Result: 在公共数据集上的实验表明，DeltaDeno实现了出色的生成效果、真实性和下游检测性能的一致提升。

Conclusion: DeltaDeno是一种有效的零样本异常生成方法，无需训练即可生成真实异常，并在下游检测任务中表现优异。
...
Conclusion: 特征域比像素域更适合信息补偿，在放大分辨率下进行变形能更好地保留高频信息，所提出的DGAF-VSR方法在多个关键指标上实现了显著提升。
...
TL;DR: 提出一种集成语义和形状感知的深度学习框架，用于CBCT图像中的牙齿分割，特别针对牙间粘连导致的形状失真问题。
...
Result: 在复杂现实场景中性能平均下降10.4%，特别是在小/遮挡对象和复杂空间关系上。PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试上均取得一致增益。

Conclusion: OmniGround基准测试和PG-TAF框架有效解决了时空视频定位中的现有局限性，为复杂现实场景下的对象定位提供了更可靠的解决方案。
...
TL;DR: 提出了一种联合建模动态图像序列和运动场的隐式神经表示框架，通过光流方程进行物理启发的正则化，无需预先估计运动场即可同时恢复时间相干图像和运动场。


<details>
  <summary>Details</summary>
Motivation: 传统运动补偿重建方法依赖预先估计的光流，但在欠采样情况下不准确且会降低重建质量，需要一种能同时恢复图像和运动场的方法。

Method: 使用两个隐式神经表示：一个参数化时空图像内容，另一个表示光流场，通过光流方程进行耦合作为物理正则化，并结合数据一致性损失。

Result: 在动态心脏MRI数据集上的实验表明，该方法优于最先进的运动补偿和深度学习方法，实现了更优的重建质量、准确的运动估计和改进的时间保真度。

Conclusion: 隐式联合建模与流正则化约束在推进动态MRI重建方面具有潜力，能够同时恢复高质量的图像和运动场。
...
TL;DR: 本文提出了FingerCap任务，旨在生成捕捉手部动作详细手指级语义的文本描述，并构建了包含4万对手部运动视频和字幕的大规模数据集FingerCap-40K。为了解决视频MLLMs的时间稀疏性问题，提出了FiGOP方法，通过配对RGB关键帧和后续手部关键点来恢复精细时间线索。


<details>
  <summary>Details</summary>
Motivation: 理解细粒度人类手部运动对视觉感知、具身智能和多模态通信至关重要。现有方法在捕捉手指级语义方面存在困难，特别是由于时间稀疏性导致无法捕捉细微的高频手指动态。

Method: 提出了FiGOP方法，将每个RGB关键帧与后续手部关键点配对，直到下一个关键帧。使用轻量级时间编码器将关键点转换为运动嵌入，并与RGB特征集成。FiGOP将经典GOP概念应用于手指运动，在不增加RGB密度的情况下恢复精细时间线索。

Result: 在FingerCap-40K数据集上的实验表明，强大的开源和闭源视频MLLMs在手指级推理方面仍然困难，而FiGOP增强模型在HandJudge和人工评估中都取得了持续改进。

Conclusion: FiGOP方法有效解决了视频MLLMs在捕捉细粒度手指运动时的时间稀疏性问题，为手指级手部运动描述提供了有效的解决方案。
...
TL;DR: 本文提出了一种基于点监督的面部表情定位方法，通过双分支框架解决单时间戳标注下的表情实例识别问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖完整的时间边界标注，成本高昂且耗时。本文旨在减少标注需求，仅需每个表情实例的单个时间戳标注即可进行训练。

Method: 提出双分支框架：1）基于高斯分布的实例自适应强度建模模块进行软伪标签分配；2）类别感知的顶点分类分支区分宏表情和微表情。还引入了强度感知对比损失来增强特征学习。

Result: 在SAMM-LV、CAS(ME)^2和CAS(ME)^3数据集上的广泛实验证明了所提框架的有效性。

Conclusion: 该方法成功实现了仅需点监督的面部表情定位，通过软伪标签和双分支设计有效解决了中性帧与不同强度表情帧的混淆问题。
...
TL;DR: MatPedia是一个基于联合RGB-PBR表示的基础模型，通过将材料编码为RGB外观和PBR地图的相互依赖潜在空间，使用视频扩散架构统一处理多种材料任务，在质量和多样性上超越现有方法。
...
Method: 提出新颖的联合RGB-PBR表示，将材料编码为两个相互依赖的潜在空间：RGB外观和四个PBR地图。将它们表述为5帧序列，采用视频扩散架构来捕捉相关性并迁移RGB生成模型的视觉先验。

Result: 在MatHybrid-410K混合数据集上训练，MatPedia实现了原生1024×1024合成，在质量和多样性上显著超越现有方法。

Conclusion: MatPedia提供了一个统一的框架，能够处理文本到材料生成、图像到材料生成和内在分解等多种材料任务，通过联合表示实现了更好的材料合成效果。
...
TL;DR: 提出了一种边缘设备上基于烹饪状态和配方的食物图像生成方法，通过引入烹饪进展数据集和烹饪图像相似度指标，显著提升了生成图像的真实性和时间一致性。
...
Method: 提出了第一个基于烤箱的烹饪进展数据集，包含厨师标注的熟度等级；开发了边缘高效的配方和烹饪状态引导生成器，基于原始食物图像合成逼真图像；引入了领域特定的烹饪图像相似度（CIS）指标作为训练损失和进度监控信号。

Result: 模型在多个数据集上显著优于现有基线方法，FID分数分别提升了30%（在自建数据集）和60%（在公共数据集）。

Conclusion: 该方法能够根据用户偏好的视觉目标而非固定预设生成逼真的烹饪食物图像，同时确保时间一致性和烹饪合理性。
...
TL;DR: 本文提出了SeeCLIP框架，通过细粒度语义增强解决开放集域泛化中的结构风险和开放空间风险困境，特别针对与已知类具有视觉相似性的"硬未知"类别。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理域偏移和新类别时面临结构风险与开放空间风险的权衡困境，容易产生过度自信，特别是在区分与已知类具有细粒度视觉相似性的"硬未知"类别时。
...
TL;DR: 提出受自然选择启发的3DGS剪枝框架，通过将生存压力建模为应用于不透明度的正则化梯度场，让优化梯度自主决定保留或剪除哪些高斯元素，无需人工干预。
...
Conclusion: 该方法完全可学习，无需人工干预，为3DGS提供了一种有效的自动剪枝解决方案。
...
TL;DR: DReX是一个纯视觉模型，通过融合自监督和卷积表示来预测图像复杂度，在IC9600基准上达到SOTA性能，超越多模态方法且参数更少。


<details>
  <summary>Details</summary>
Motivation: 探索语言信息是否对图像复杂度预测任务必要，以及视觉特征本身是否足以进行人类对齐的复杂度预测。
...
Conclusion: 视觉特征本身足以进行人类对齐的复杂度预测，自监督transformer和监督CNN融合能提供互补和协同效益。
...
TL;DR: DepthFocus是一个可控制的视觉Transformer模型，将立体深度估计重新定义为意图驱动的控制，能够根据深度偏好动态调整计算以聚焦到目标深度层。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的深度很少是单一的，透射性材料会产生分层模糊，而现有模型只能被动估计最近表面的静态深度图，无法像人类那样主动调整焦点来感知所需深度。

Method: 引入DepthFocus，一个可控制的视觉Transformer，通过标量深度偏好作为条件，动态调整计算以聚焦到目标深度层。训练主要使用新构建的50万张多层合成数据集，捕捉多样化的透视效果。

Result: 在BOOSTER等传统单深度基准测试中达到最先进性能，在新提出的真实和合成多深度数据集上定量展示了意图对齐的估计能力，并在未见过的透视场景中表现出强大的泛化能力。

Conclusion: DepthFocus代表了向主动且类人3D感知的重要一步，展示了在复杂场景中进行选择性感知的能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶和户外机器人在雨、雾、雪等恶劣天气条件下可靠视觉感知的挑战，需要一种能处理不同退化程度和多种天气条件的统一恢复模型。

Method: 提出MVLR模型，将轻量级编码器-解码器主干与视觉语言模型和隐式记忆库耦合。VLM通过思维链推理编码天气退化先验，IMB存储退化模式的连续潜在表示，通过动态交叉注意力机制自适应融合多尺度视觉特征。

Result: 在四个恶劣天气基准测试上的广泛实验表明，MVLR在PSNR和SSIM指标上超越了单分支和专家混合基线方法。

Conclusion: MVLR在模型紧凑性和表达能力之间提供了实用平衡，适用于各种户外条件下的实时部署。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中直接替换身份信息来实现人脸去识别，利用化学中的置换机制概念，在潜在h空间中进行语义位移。


<details>
  <summary>Details</summary>
Motivation: 解决现有去识别方法在身份抑制和属性保留之间的权衡问题，寻求更优的平衡点。

Method: 通过优化引导发现身份编辑方向，使用新颖的试剂损失函数监督属性保留和身份抑制，提出线性和测地线编辑方案在潜在流形中导航。

Result: 在CelebA-HQ和FFHQ数据集上的实验表明，FLUID在身份抑制和属性保留方面实现了优越的权衡，在定性和定量指标上都优于现有最先进的去识别方法。

Conclusion: FLUID框架通过潜在空间的身份置换机制，为图像去识别任务提供了一种有效且无需训练的解决方案。
...
TL;DR: ConfusedTourist是一个评估视觉语言模型文化对抗鲁棒性的新基准，发现模型在面临混合文化线索时准确率显著下降，注意力会系统性转向干扰线索。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常每张图像只包含单一文化概念，忽视了多个可能不相关的文化线索共存的情况，而模型在这种文化概念混合场景下的稳定性尚未得到充分测试。

Method: 引入ConfusedTourist文化对抗鲁棒性测试套件，通过图像堆叠和基于图像生成的扰动来评估VLMs对地理线索扰动的稳定性，并进行可解释性分析。

Result: 实验显示模型在简单图像堆叠扰动下准确率大幅下降，基于图像生成的扰动版本表现更差。可解释性分析表明失败源于注意力系统性转向干扰线索。

Conclusion: 视觉文化概念混合会严重损害最先进的VLMs，凸显了开发更具文化鲁棒性的多模态理解模型的迫切需求。
...
TL;DR: RacketVision是一个用于体育分析的新型计算机视觉数据集和基准，涵盖乒乓球、网球和羽毛球，提供大规模细粒度球拍姿态标注和传统球位置标注，支持球跟踪、球拍姿态估计和球轨迹预测三个任务的研究。


<details>
  <summary>Details</summary>
Motivation: 推动体育分析中的计算机视觉研究，特别是针对复杂人-物交互的研究，填补了现有数据集在球拍姿态细粒度标注方面的空白。

Method: 构建包含乒乓球、网球和羽毛球的大规模数据集，提供球拍姿态和球位置的细粒度标注，采用CrossAttention机制进行多模态融合，而不是简单的特征拼接。
...
<details>
  <summary>Details</summary>
Motivation: LVLMs在图像级任务表现出色，但在实例级任务（如视觉定位和目标检测）中仍存在性能差距。行人跟踪任务需要模型在高级语义层面理解跟踪对象，这正是LVLMs的优势所在。

Method: 采用RL-Mid Training-SFT-RL训练阶段：基于LVLM预训练权重，先进行RL阶段使模型输出固定可监督的边界框格式；然后使用大量行人相关数据集进行中间训练；最后在多个行人跟踪数据集上进行监督微调，再进行另一个RL阶段以提升跟踪性能和指令跟随能力。

Result: 在跟踪基准测试上的实验结果表明，所提方法能够比先前方法表现更好。

Conclusion: OmniPT框架成功地将行人跟踪任务建模为基础模型可执行的任务，并实现了优于先前方法的性能。
...
TL;DR: ReBrain是一个基于检索增强扩散的框架，用于从稀疏CT扫描重建完整脑部MRI体积，通过检索相似CT切片和使用布朗桥扩散模型来确保结构连续性。
...
Method: 使用布朗桥扩散模型合成2D MRI切片，同时通过微调检索模型从先验数据库中检索结构和病理相似的CT切片，通过ControlNet分支指导中间MRI切片生成，并对罕见检索失败情况应用球面线性插值。
...
TL;DR: STAR建模通过生成图像块位置的均匀生成树遍历顺序，在保持采样性能的同时提供灵活的序列顺序，以支持图像编辑。
...
Result: STAR在保持采样性能的同时提供了后补完成的灵活性，无需对广泛采用的语言自回归模型架构进行重大更改。

Conclusion: STAR通过定制但结构化的随机策略，在视觉生成中成功平衡了采样性能和序列顺序灵活性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法REArtGS在处理螺旋关节或多部件物体时仍存在困难，且缺乏对未见状态的几何约束。

Method: 采用解耦螺旋运动建模关节，通过部件运动融合联合优化部件感知高斯和关节参数；引入平面高斯和基于泰勒一阶展开的时序一致性正则化。

Result: 在合成和真实世界铰接物体上的大量实验表明，该方法在部件级表面重建和关节参数估计方面优于现有方法。

Conclusion: REArtGS++通过时序几何约束和平面高斯抛光实现了更通用的铰接物体重建，显著提升了重建质量和关节参数估计精度。
...
### [40] [Planning with Sketch-Guided Verification for Physics-Aware Video Generation](https://arxiv.org/abs/2511.17450)
*Yidong Huang,Zun Wang,Han Lin,Dong-Ki Kim,Shayegan Omidshafiei,Jaehong Yoon,Yue Zhang,Mohit Bansal*

Main category: cs.CV

TL;DR: SketchVerify是一个无需训练的基于草图验证的运动规划框架，通过测试时采样和验证循环来生成更动态一致的运动轨迹，在完整视频生成前提高运动质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法使用单次规划或迭代精炼，前者运动简单，后者计算成本高。需要一种既能生成复杂动态运动又高效的方法。

Method: 提出草图验证框架：预测多个候选运动轨迹，使用视觉语言验证器评估语义对齐和物理合理性，通过将轨迹渲染为轻量级视频草图来高效评分，迭代精炼直到找到满意方案。

Result: 在WorldModelBench和PhyWorldBench上的实验表明，该方法显著提高了运动质量、物理真实性和长期一致性，同时效率更高。增加轨迹候选数量能持续提升性能。

Conclusion: SketchVerify框架通过高效的草图验证机制，在保持计算效率的同时显著提升了视频生成中的运动规划质量。
...
TL;DR: DiverseVAR是一种无需额外训练即可恢复视觉自回归模型生成多样性的方法，通过抑制输入中的关键特征组件并放大输出中的该组件来提升多样性，同时保持高质量合成。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型虽然推理效率高、图像质量好，但存在多样性崩溃问题，类似于少步蒸馏扩散模型中的多样性减少现象。

Method: 通过分析发现特征图中的关键组件是早期尺度多样性形成的关键因素，在模型输入中抑制该关键组件，在输出中放大该组件。

Result: 实验结果表明该方法显著提升了生成多样性，且对性能影响可忽略不计。

Conclusion: DiverseVAR有效释放了VAR模型的固有生成潜力，在保持高保真合成的同时解决了多样性崩溃问题。
...
Method: 首先引入高斯信息场感知最优稀疏视角，然后将3D高斯压缩为平面高斯以准确估计法线和深度，通过深度平滑正则化和少样本扩散进行粗到精优化，并为每个高斯基元引入部件分割概率。
...
Method: 提出ReCoVAD框架，包含两个核心通路：反射通路使用轻量级CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；意识通路使用中规模视觉语言模型生成文本事件描述和精炼异常分数，通过集成大语言模型定期审查识别未见异常。

Result: 在UCF-Crime和XD-Violence数据集上实现最先进的免训练性能，同时仅处理先前方法所用帧数的28.55%和16.04%。

Conclusion: 稀疏推理对于基于大模型的有效视频异常检测是足够的，ReCoVAD通过选择性帧处理显著减少了计算成本。
...
TL;DR: 提出分区自适应对比学习（PACL）方法，利用预训练文本模型的知识来增强预训练视觉模型的情感感知能力，解决视觉情感识别中的"情感鸿沟"问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态推理模型在生成长推理链时存在冗余自反，而基于静态视觉参考的CoT压缩方法对多模态推理增益有限。

Method: ChainV首先基于前一步推理进行粗粒度视觉补丁选择，然后通过平均注意力强度识别最具代表性的原子视觉提示，并引入一致性评估机制来调整自反水平。

Result: 在MathVista基准测试中准确率提升2.3%，推理延迟降低51.4%，输出token长度缩短24.5%。

Conclusion: ChainV通过动态整合视觉提示显著提高了多模态推理的准确性和效率，特别在数学密集型任务中表现突出。
...
TL;DR: 提出了一个与LVLMs因果架构对齐的综合干预框架，通过分析图像-输入文本、图像-输出文本和文本-文本路径的相互作用来减少幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型在多种任务中表现出色，但仍容易产生幻觉。本研究旨在深入理解LVLMs中幻觉产生的多路径机制。

Method: 提出了一个综合干预框架，识别并干预每个路径中的关键幻觉头，针对判别式和生成式格式进行定制化处理。

Result: 实验表明该方法在多个基准测试中持续减少不同对齐类型的幻觉。

Conclusion: LVLMs中的幻觉并非来自单一因果路径，而是多路径相互作用的结果，且模型依赖的路径取决于问答对齐格式。
...
TL;DR: PEGS是一个将物理先验与事件流增强集成到3D高斯泼溅管道中的框架，用于执行去模糊的目标聚焦建模和运动恢复，通过三重监督方案和运动感知模拟退火策略，在重建大时空尺度运动方面表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 重建大时空尺度上的刚性运动仍然具有挑战性，主要由于建模范式限制、严重运动模糊和物理一致性不足。

Method: 提出PEGS框架，集成物理先验和事件流增强到3D高斯泼溅管道中，采用三重监督方案（加速度约束、事件流高时间分辨率指导、卡尔曼正则化）和运动感知模拟退火策略。

Result: 实验表明PEGS在重建大时空尺度运动方面优于主流动态方法。

Conclusion: PEGS通过物理先验和事件流增强的集成，成功解决了大时空尺度刚性运动重建的挑战，并贡献了首个针对自然快速刚性运动的RGB-Event配对数据集。
...
### [48] [Range-Edit: Semantic Mask Guided Outdoor LiDAR Scene Editing](https://arxiv.org/abs/2511.17269)
*Suchetan G. Uppur,Hemant Kumar,Vaibhav Kumar*

Main category: cs.CV

TL;DR: 提出了一种基于语义掩码引导的LiDAR点云编辑方法，通过将点云转换为2D距离图像并使用语义掩码指导扩散生成，能够高效生成复杂边缘案例的合成LiDAR数据。
...
TL;DR: 本文提出从1D相关色温(CCT)映射转向2D色度空间，并使用轻量级多层感知机(MLP)替代传统CCT插值，以改善非普朗克光源(如LED)下的色度映射精度。
...
Method: 使用2D色度空间特征训练轻量级MLP，结合包含代表性LED光源的光箱校准程序，实现非普朗克光源下的鲁棒色度映射。

Result: 在多种LED照明下验证，该方法平均减少22%的角度再现误差，保持与传统光源的向后兼容性，支持多光源场景，并可在相机中实时部署且计算成本可忽略。

Conclusion: 从1D CCT转向2D色度空间并结合MLP方法，显著提高了非普朗克光源下的色度映射精度，同时保持实用性和兼容性。
...
### [50] [Where Culture Fades: Revealing the Cultural Gap in Text-to-Image Generation](https://arxiv.org/abs/2511.17282)
...
TL;DR: 本文分析了多语言文本到图像模型在文化一致性方面的问题，发现现有模型在多语言提示下往往产生文化中性或英语偏见的结果。作者提出了一种定位文化敏感神经元的方法，并开发了两种对齐策略来增强文化一致性。


<details>
  <summary>Details</summary>
Motivation: 多语言文本到图像模型虽然在视觉真实性和语义对齐方面进步迅速，但由于语言承载文化内涵，模型在不同语言提示下应保持跨语言文化一致性。然而当前模型往往产生文化中性或英语偏见的结果，这影响了模型的文化适应性。

Method: 1) 提出一种探测方法，将文化敏感信号定位到少数固定层中的一小部分神经元；2) 基于此发现引入两种互补的对齐策略：推理时文化激活（无需主干微调）和层定向文化增强（仅更新文化相关层）。
...
TL;DR: 提出一个完整的多阶段优化框架，将高性能浮点图像压缩模型转换为高效的8位整数FPGA实现，通过动态范围感知量化、混合精度搜索和通道剪枝等技术，在保持性能的同时大幅降低计算复杂度。
...
Result: DRAQ方法将GDN模型的BD-rate开销从30%降至6.3%；硬件感知优化进一步降低超过20%的计算复杂度，对RD性能影响可忽略，最终模型在效率和性能上都优于现有FPGA实现。

Conclusion: 该框架成功实现了高性能深度学习图像压缩模型向高效FPGA部署的转换，为资源受限环境下的模型部署提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散基真实世界图像超分辨率方法在保真度和可控性之间存在平衡问题：多步方法由于生成多样性和随机性导致保真度低，而一步方法因保真度特定微调而失去控制灵活性。

Method: 提出噪声混合视觉流设计：新引入的视觉流接收带可调噪声的低质量图像，原始视觉流接收带一致噪声的低质量图像；采用保真度感知对抗训练实现一步推理。

Result: 在通用真实世界图像超分辨率任务上达到最先进性能，并在中文场景文本图像超分辨率等挑战性场景中实现提示可控性，无需在特定数据集上训练。
...
TL;DR: 本文提出MuM模型，通过将掩码自编码（MAE）扩展到场景的任意多视图，使用统一掩码和轻量级解码器，在3D视觉任务中超越了DINOv3和CroCo v2。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法主要优化语义理解而非几何推理，CroCo虽然针对3D理解但仍有改进空间。本文旨在继续CroCo的研究路径，专注于学习更适合3D视觉的特征。

Method: 扩展MAE到任意多视图，通过统一掩码所有视图并使用具有帧间注意力的轻量级解码器，方法比CroCo更简单且更具可扩展性。

Result: 在包括前向重建、密集图像匹配和相对姿态估计等下游任务中，MuM模型超越了当前最先进的视觉编码器DINOv3和CroCo v2。

Conclusion: MuM方法通过多视图掩码自编码有效学习3D视觉特征，在多个3D视觉任务中表现出色，证明了其在3D理解方面的优势。
...
TL;DR: CC-DiceCE损失函数基于CC-Metrics框架，旨在解决传统Dice损失对小病灶分割不足的问题，在nnU-Net框架下相比blob loss和DiceCE基线提高了检测召回率，但略微增加了假阳性。


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割损失函数（如Dice）由于小病灶相对体积小而对整体损失贡献可忽略，导致对小病灶分割不足。需要实例级别的损失函数来评估每个病灶的分割质量。

Method: 基于CC-Metrics框架提出CC-DiceCE损失函数，在nnU-Net标准化框架下与现有的blob loss和DiceCE基线进行对比评估。

Result: CC-DiceCE损失提高了检测召回率，分割性能仅有最小程度下降，但略微增加了假阳性。在多数据集研究中，CC-DiceCE通常优于blob loss。

Conclusion: CC-DiceCE损失函数能有效提升小病灶检测能力，在保持分割性能的同时提高召回率，是优于现有blob loss的解决方案。
...
Method: 整合先进生成模型创建多样化的高质量合成人脸图像，强调代表各种面部特征，并确保符合身份证照片的许可特征。

Result: 分析显示现有验证模型对某些性别和种族存在偏见，身份风格修改会负面影响模型性能。

Conclusion: 通过解决现有数据集中的固有不平等问题，这项工作不仅丰富了人工智能多样性和伦理的讨论，还为开发更包容可靠的面部验证技术奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成式方法依赖人工设计的轨迹锚点或随机噪声，存在改进空间，需要更灵活的轨迹预测方法。

Method: 第一阶段使用Transformer-based Proposal Decoder生成粗略轨迹预测，第二阶段应用Diffusion Refiner迭代去噪和精炼初始预测，并设计了细粒度去噪解码器增强场景合规性。
...
TL;DR: UI-Styler是一种针对超声图像的类别感知风格迁移框架，通过模式匹配机制将目标图像纹理模式转移到源图像，同时保留源图像结构内容，并使用类别感知提示策略确保语义对齐。
...
Method: 提出UI-Styler框架，包含：1）模式匹配机制，将目标图像纹理模式转移到源图像；2）类别感知提示策略，基于目标域伪标签指导，确保与诊断类别的准确语义对齐。

Result: 在超声跨设备任务上的大量实验表明，UI-Styler在分布距离和下游任务（如分类和分割）上始终优于现有UIT方法，达到最先进性能。

Conclusion: UI-Styler通过有效的风格迁移和语义对齐，成功解决了超声图像跨设备域适应问题，显著提升了黑盒推理模型的性能。
...
Method: 提出FireScope-Bench数据集和基准，结合哨兵2号影像、气候数据和专家定义的风险栅格。开发FireScope框架，基于VLM的推理到生成方法，通过强化学习和视觉监督学习预测风险栅格并生成互补的推理轨迹。

Result: 在美国训练并在欧洲测试时，FireScope实现了显著的性能提升。专家反馈和自动分析确认其推理轨迹是忠实且语义上有意义的。

Conclusion: 推理可以基础化栅格预测模型，提高泛化能力和可解释性。这是首个证明基于语言的推理可以改善视觉生成泛化性的框架，也是首个可跨大陆应用的高分辨率野火风险模型。
...
TL;DR: 本文系统评估了自监督表示在音频-视觉深度伪造检测中的应用，发现这些特征能捕捉到与深度伪造相关的信息且具有互补性，但无法可靠地在不同数据集间泛化。


<details>
  <summary>Details</summary>
Motivation: 探索自监督表示在音频-视觉深度伪造检测中的潜力，此前的研究要么孤立使用这些特征，要么将其嵌入复杂架构中，缺乏系统性评估。

Method: 在多种模态（音频、视频、多模态）和领域（唇部运动、通用视觉内容）上系统评估自监督特征，重点关注检测效果、编码信息的可解释性和跨模态互补性三个维度。

Result: 大多数自监督特征能捕捉深度伪造相关信息且信息具有互补性，模型主要关注语义上有意义的区域而非虚假伪影，但所有模型都无法在不同数据集间可靠泛化。
...
TL;DR: 本文介绍了RSFM数据库和REMSA智能体，用于解决遥感基础模型选择困难的问题，通过结构化数据库和LLM驱动的智能体实现自动化模型推荐。
...
Method: 构建了覆盖150多个遥感基础模型的RSFM数据库，并开发了基于LLM的REMSA智能体，通过自然语言查询解析、约束补全、上下文学习排序和透明解释来推荐模型。

Result: 在75个专家验证的查询场景和900种配置下，REMSA在模型选择任务上优于多种基线方法，包括朴素智能体、密集检索和非结构化RAG方法。
...
Conclusion: PostCam通过统一的特征表示和两阶段训练策略，实现了更准确和灵活的相机运动操控，显著提升了动态场景中新颖视角视频的生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型在需要细粒度表示的时空一致性操作任务中面临挑战，特别是在实现动作执行的时间一致性控制方面存在困难。

Method: 提出4D感知的视觉表示（将1D时间嵌入3D位置形成4D嵌入，通过交叉注意力机制融合）和时空动作表示（扩展传统空间动作表示加入时间信息），并将多模态表示对齐到LLM中进行时空动作预测。

Result: 在多个机器人操作任务上的广泛实验验证了该方法的优越性，能够实现空间平滑和时间一致的操作。

Conclusion: VLA-4D模型通过统一的4D感知框架，成功实现了机器人操作的时空一致性控制，在各项任务中表现出色。
...
TL;DR: 本文提出CA-SAM方法，通过轻量级的对齐层模块和持续学习策略，在保持SAM模型零样本能力的同时，提高医学图像分割的计算效率和性能，解决异构隐私政策下的持续学习问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，机构间的异构隐私政策使得联合训练不可行，需要从数据流中进行持续学习以避免灾难性遗忘。SAM模型虽然具有强大的零样本能力，但参数量大、计算开销高，限制了实际部署。

Method: 提出对齐层（Alignment Layer）作为轻量级即插即用模块，对齐编码器-解码器特征分布；在此基础上提出CA-SAM持续学习策略，自动适配对齐层来缓解灾难性遗忘，同时利用SAM的零样本先验。

Result: 在九个医学分割数据集的持续学习场景下，CA-SAM取得了最先进的性能表现。

Conclusion: SAM范式在平衡计算效率和性能后具有很大潜力，CA-SAM通过轻量级对齐层和持续学习策略，有效解决了医学图像分割中的持续学习问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有稠密3D重建方法虽然能准确捕捉局部几何，但集成到SLAM中面临漂移和冗余点云地图的问题，限制了效率和下游任务（如新视角合成）的性能。

Method: 首先通过轻量级跟踪和重建模块构建局部一致的子地图，然后逐步对齐和融合到全局高斯地图中，强制跨视角几何一致性，全局地图反过来纠正局部漂移并增强跟踪鲁棒性。

Result: 在真实世界数据集上，SING3R-SLAM实现了最先进的跟踪、3D重建和新视角渲染性能，跟踪精度提升超过12%，生成更精细、更详细的几何结构，同时保持紧凑和内存高效的全局表示。

Conclusion: SING3R-SLAM通过全局一致的高斯表示有效解决了稠密SLAM中的漂移和冗余问题，为多种下游应用提供了高效且多功能的3D建图解决方案。
...
Method: 使用可扩展的3D视觉Transformer架构，结合DINO风格的自蒸馏和基于SigLIP的视觉语言对齐，联合优化局部Transformer进行高分辨率特征提取和全局Transformer进行全扫描上下文建模。

Result: 在多个CT基准测试中，SPECTRE在零样本和微调设置下均优于先前的CT基础模型，证明无需私有数据也能获得高性能的通用表示。

Conclusion: SPECTRE是一个可扩展、开放且完全基于Transformer的3D医学影像基础模型，能够学习几何一致且临床有意义的特征表示。
...
TL;DR: 提出了一种针对鱼眼图像的BEV语义分割方法，通过几何反投影和像素级深度分布估计，使用高斯参数化建模几何不确定性，无需去畸变即可生成连续的BEV语义地图。
...
Method: 使用多相机高分辨率鱼眼图像，通过几何反投影和像素级深度分布估计，将每个像素通过高斯参数化提升到3D空间，预测空间均值和各向异性协方差来显式建模几何不确定性，然后通过可微splatting融合到BEV表示中。

Result: 在复杂停车和城市驾驶场景中表现出色，在严重鱼眼畸变和多样化环境条件下，可行驶区域IoU达到87.75%，车辆IoU达到57.26%。

Conclusion: 该方法能够有效处理鱼眼图像的畸变问题，无需去畸变或透视校正即可生成连续、不确定性感知的BEV语义地图，在复杂场景中实现了强大的分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 由于大规模3D标注成本高昂，现有方法要么依赖2D渲染一致性（3D结构仅隐式出现），要么依赖离散化的体素网格（限制空间精度和可扩展性），需要开发更有效的自监督3D语义占据学习方法。

Method: 引入基于独立4D时空查询的自监督框架，支持从伪点云或原始激光雷达数据监督；提出压缩场景表示，在保持近场细节的同时平滑压缩远距离区域，实现恒定内存下的长距离推理。
...
TL;DR: 本文提出了一种针对彩色图像盲反卷积的新方法，通过四元数保真项处理彩色通道间的关系，使用归一化四元数核来保持图像强度，实验证明该方法能有效去除伪影并显著改善去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有彩色图像盲反卷积方法通常将彩色图像转换为灰度图像或单独处理每个颜色通道，忽视了颜色通道之间的关系，这限制了去模糊效果。

Method: 提出了一种新颖的四元数保真项，利用四元数卷积核的特性，包含四个核：一个用于捕获整体模糊的非负卷积核，以及三个分别对应红、绿、蓝通道的无约束卷积核来建模它们之间的未知相互依赖关系。使用归一化四元数核来保持图像强度。

Result: 在真实模糊彩色图像数据集上的广泛实验表明，所提出的方法能有效去除伪影，并显著改善去模糊效果。

Conclusion: 该方法展示了作为彩色图像反卷积强大工具的潜力，通过考虑颜色通道间的关系显著提升了盲反卷积性能。
...
TL;DR: 本文提出了一种基于相关性反馈的视觉语言模型检索改进方法，包括伪相关性反馈、生成相关性反馈、注意力反馈总结器和显式反馈四种策略，能够在不进行微调的情况下提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型支持自然语言查询的视觉搜索，但提升性能通常需要微调或使用更大模型。本文旨在通过相关性反馈机制在推理时改进检索性能，作为微调的替代方案。

Method: 提出了四种反馈策略：1）伪相关性反馈（PRF）基于排名靠前结果优化查询嵌入；2）生成相关性反馈（GRF）使用合成标题进行查询优化；3）注意力反馈总结器（AFS）整合多模态细粒度特征的自定义transformer模型；4）使用真实标题的显式反馈作为上限基准。

Result: 在Flickr30k和COCO数据集上的实验显示，GRF、AFS和显式反馈相比无反馈检索，对较小VLM提升MRR@5 3-5%，对较大VLM提升1-3%。AFS与显式反馈类似，能够减轻查询漂移，在迭代多轮检索设置中比GRF更稳健。
...
<details>
  <summary>Details</summary>
Motivation: 现有OCSR系统在准确识别立体化学信息方面面临挑战，如楔形键、虚线键等细微视觉线索难以区分立体异构体。

Method: 采用三阶段训练范式：1)大规模噪声数据集预训练；2)多粒度微调，探索化学键分类和原子定位辅助任务；3)强化学习后训练优化，引入新的立体化学结构数据集。

Result: 实验表明MolSight在立体化学光学结构识别方面达到最先进性能，即使参数规模相对紧凑，GRPO算法也能进一步提升立体分子识别能力。

Conclusion: MolSight框架通过系统化的训练策略成功解决了OCSR中立体化学信息识别的关键挑战，为化学信息学应用提供了可靠的工具。
...
Method: 提出双模态输入方法，包括从触摸屏获取的电容图像和屏下指纹传感器获取的指纹补丁，通过结合两种模态信息实现可靠的滚转角估计和其他姿态参数的改进预测。

Result: BiFingerPose相比之前SOTA方法，预测性能提升超过21%，任务完成效率提高2.5倍，用户操作准确性提升23%。在12人用户研究中验证了方法的优势。

Conclusion: 该方法在增强认证安全性和改善交互体验方面具有应用潜力，开发的相应原型展示了交互可能性。
...
TL;DR: 本文提出SpatialGeo方法，通过融合几何和语义特征来增强多模态大语言模型的空间推理能力，解决了现有模型在三维空间理解方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 大多数多模态大语言模型在解释和推断三维空间空间排列方面存在空间推理能力有限的问题，这源于视觉编码器的空间模糊性缺陷。

Method: 提出基于几何和语义特征分层融合的新型视觉编码器，通过层次适配器将自监督学习的几何特征与CLIP的语义特征相结合，并使用随机特征丢弃避免对CLIP编码器的依赖。

Result: SpatialGeo在空间推理任务中显著提升准确率，在SpatialRGPT-Bench上将最先进模型的准确率提高了至少8.0%，同时推理期间内存成本减少约50%。

Conclusion: 通过融合几何和语义特征的方法有效增强了多模态大语言模型的空间感知能力，在保持高效性的同时显著提升了空间推理性能。
...
Method: 采用相对姿态初始化与显式特征匹配，进行局部联合优化增强姿态估计，然后引入包含特征轨迹的全局捆绑调整，整合几何一致性约束。

Result: 在基准数据集上表现出优越性能和鲁棒性，在姿态估计精度和新视角合成方面均优于现有最优方法，特别是在挑战性场景中。
...
<details>
  <summary>Details</summary>
Motivation: 生成图像模型在合成透明物体时表现不佳，特别是折射效果，因为折射光线会与图像其他部分表面相交形成颜色约束，现有模型未能充分学习光学规律。
...
Conclusion: 通过引入光学物理约束和像素同步机制，显著提升了生成图像中透明物体折射效果的物理准确性。
...
TL;DR: 提出一个统一框架用于多媒介绘画过程生成，通过语义驱动的风格控制机制将多种媒介嵌入扩散模型的条件空间，实现跨媒介风格增强和一致的纹理演化。
...
Method: 使用语义驱动的风格控制机制将多种媒介嵌入扩散模型条件空间，采用跨媒介风格增强和反向绘画训练策略，确保生成过程的平滑性和与人类创作的一致性。
...
<details>
  <summary>Details</summary>
Motivation: 受人类视觉感知启发，现有I-JEPA方法对所有区域进行统一独立处理，缺乏对预测顺序和位置的显式建模，无法像人类那样从最信息丰富的区域开始选择性、顺序性地部署注意力。

Method: 首先基于Transformer衍生的显著性图识别主要判别区域，然后按判别性顺序预测后续区域，形成从主要到次要线索的课程式语义进展，整合了JEPA风格的潜在预测和GPT风格的顺序推理。

Result: 在图像分类、细粒度视觉分类、检测分割和低级推理任务上的广泛实验表明，DSeq-JEPA比I-JEPA变体始终关注更具判别性和泛化性的表示。

Conclusion: DSeq-JEPA成功桥接了预测性和自回归自监督学习，通过引入判别性顺序预测机制，显著提升了视觉表示的质量和泛化能力。
...
TL;DR: SuperQuadricOcc提出了一种基于超二次曲面的语义占据估计方法，相比高斯表示方法减少了84%的基元数量，实现了75%的内存占用降低和124%的推理速度提升，同时mIoU提高了5.9%。
...
Method: 采用基于超二次曲面的场景表示，利用多层二十面体细分的高斯近似来实现训练期间的监督，通过高斯光栅化进行监督。
...
TL;DR: 提出了ATAC方法，通过在CLIP嵌入空间中计算增强诱导漂移向量来推断语义恢复方向，显著提升了CLIP对抗扰动的鲁棒性，平均超越先前方法近50%


<details>
  <summary>Details</summary>
Motivation: CLIP在零样本图像-文本匹配方面表现出色，但对图像对抗扰动高度脆弱。由于对抗微调成本过高，现有测试时防御方法鲁棒性有限

Method: ATAC方法直接在CLIP嵌入空间中操作，计算增强诱导漂移向量来推断语义恢复方向，基于这些潜在漂移的角度一致性来校正嵌入

Result: 在广泛基准测试中，ATAC始终实现显著高鲁棒性，平均超越先前最先进方法近50%，同时计算开销极小。在非常规和极端设置下保持最先进鲁棒性，甚至对自适应攻击也实现非平凡鲁棒性

Conclusion: ATAC是CLIP嵌入空间中测试时对抗防御新范式的有效方法，展示了在嵌入空间进行对抗校正的潜力
...
TL;DR: 本文提出SVRecon方法，将稀疏体素栅格化范式扩展到高保真表面重建任务，通过集成有符号距离函数(SDF)来解决稀疏体素优化中的局部最小值问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏体素在空间上与其邻居解耦且具有锐利边界，这使它们在优化过程中容易陷入局部最小值。虽然SDF值提供了自然平滑连续的几何场，但在独立参数化的稀疏体素之间保持这种平滑性具有挑战性。

Method: 通过(1)使用视觉几何模型进行鲁棒的几何初始化，(2)实施空间平滑损失来强制父-子和兄弟体素组之间的连贯关系，促进连贯平滑的体素结构。

Result: 在各种基准测试上的广泛实验表明，该方法实现了强大的重建精度，同时保持了一致的快速收敛速度。

Conclusion: SVRecon方法在稀疏体素表面重建中有效解决了局部最小值问题，通过SDF集成和空间平滑约束实现了高质量的重建结果和快速收敛。
...
Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上的实验验证，与现有方法相比，NPPR提供更保守(低40%)的PR估计。

Conclusion: NPPR是一个更实用的鲁棒性度量，能够在分布不确定性下提供保守的概率鲁棒性评估。
...
TL;DR: MorphSeek是一个用于可变形图像配准的细粒度表示级策略优化框架，通过在高维潜在特征空间中建模连续优化过程，解决了传统方法在捕捉空间变化变形方面的限制。


<details>
  <summary>Details</summary>
Motivation: 传统可变形图像配准方法面临高维变形空间和体素级监督稀缺的挑战，现有强化学习框架通常将空间投影到粗糙的低维表示，限制了捕捉空间变化变形的能力。
...
<details>
  <summary>Details</summary>
Motivation: 现实应用中部分模态经常在推理阶段不可用，现有多模态模型无法处理模态缺失问题，且模态缺失会因跨模态交互中断导致性能灾难性下降。
...
Conclusion: MCMoE框架有效解决了多模态AQA中的模态缺失问题，通过统一的单阶段训练实现了鲁棒的多模态动作质量评估。
...
Conclusion: MMT-ARD框架有效提升了多模态大模型的对抗鲁棒性，展示了其有效性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要基于事实观察进行前向模拟，但许多应用需要回答反事实查询，如"如果移除这个物体会发生什么？"，因此需要能够处理假设性修改的反事实世界模型。
...
TL;DR: Radar2Shape是一个去噪扩散模型，通过将雷达信号频率与多分辨率形状特征关联，从部分可观测的雷达信号进行3D重建。该方法采用两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后通过以类似粗到细的方式调节雷达信号频率来扩散到该潜在空间。
...
Method: 两阶段方法：1) 学习具有层次分辨率形状特征的规范化潜在空间；2) 通过以粗到细方式调节雷达信号频率，扩散到该潜在空间。该方法将雷达信号频率与多分辨率形状特征相关联。

Result: Radar2Shape能够成功地从部分观测的雷达信号重建任意3D形状，并在两种不同仿真方法和真实世界数据上展现出鲁棒的泛化能力。

Conclusion: 该方法为高频雷达领域的3D重建提供了有效解决方案，并发布了两个合成基准数据集以促进该领域的未来研究，使类似Radar2Shape的模型能够安全地适应到真实世界雷达系统中。
...
TL;DR: 提出了一种基于计算机视觉的深度学习方法，使用超过18,000个MRI系列图像来估计脊柱年龄，并发现脊柱年龄差距（SAG）与脊柱退行性病变和生活方式因素相关。


<details>
  <summary>Details</summary>
Motivation: 脊柱是人体重要结构，容易发生与年龄相关的退行性病变，这些病变可通过MRI识别。需要开发准确的方法来评估脊柱健康状况。

Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退行性病变聚类，建立资格标准。通过消融研究选择模型，考虑数据大小、损失函数和不同脊柱区域的影响。

Result: 模型能够准确估计脊柱年龄，脊柱年龄差距（SAG）与椎间盘突出、骨赘、椎管狭窄、骨折等退行性病变相关，也与吸烟和体力劳动等生活方式因素相关。

Conclusion: 脊柱年龄差距（SAG）可作为衡量整体脊柱健康的有用生物标志物，与多种退行性病变和生活方式因素显著相关。
...
TL;DR: 本文系统分析了多模态模型中LLM规模缩减对视觉能力的影响，发现视觉能力比语言能力更易受损，并提出Extract+Think方法通过视觉提取调优和逐步推理来提升小模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态模型规模扩大带来了显著进展，但实际应用需要更小、更高效的模型。本文旨在研究LLM规模缩减如何影响多模态能力，特别是视觉能力。

Method: 采用Extract+Think方法：1) 视觉提取调优，训练模型提取指令相关的视觉细节；2) 逐步推理生成答案。

Result: 研究发现LLM规模缩减对视觉能力的影响比对语言能力的影响更大，视觉提取调优能显著提升小模型的性能表现。

Conclusion: Extract+Think方法为多模态模型的高效性能设定了新标准，通过分离视觉提取和推理过程有效解决了小模型视觉能力下降的问题。
...
TL;DR: Video-R4是一个视频推理大语言模型，通过视觉反刍机制迭代选择帧、放大信息区域、重新编码像素来更新推理状态，在文本丰富视频理解任务中取得SOTA效果。
...
TL;DR: 提出了一种新的原生3D编辑框架，通过单次前向传播直接操作3D表示，解决了现有方法在速度和3D一致性方面的局限性。
...
Result: 令牌拼接方法在参数效率上更优且性能更好，在生成质量、3D一致性和指令保真度方面超越现有2D提升方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有向量数据库检索管道依赖于扁平或单分辨率索引结构，无法适应不同用户查询所需的语义粒度变化，导致检索速度和上下文相关性之间的次优权衡。
...
Conclusion: SPI框架与现有向量数据库基础设施兼容，可立即部署到生产RAG系统中，在保证检索质量和延迟边界的同时显著提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM推理基准测试设计目标狭窄，未能将系统指标和任务特定指标整合到统一易用的框架中，无法满足用户在多推理引擎、使用场景和量化级别下的配置优化需求。

Method: 开发Bench360框架，允许用户定义自定义任务、数据集和相关指标，自动对选定的LLM、推理引擎和量化级别在不同使用场景（单流、批处理和服务器）下进行基准测试。

Result: 在四个常见LLM任务、三个硬件平台和四个最先进推理引擎上的测试结果表明，任务性能与系统级效率之间存在多个有趣的权衡关系，不同推理引擎和模型存在显著差异。
...
Method: 创建了包含突尼斯方言、标准阿拉伯语和英语的平行数据集，并带有情感标签，在转写、翻译和情感分析三个任务上对多个流行LLM进行基准测试。
...
TL;DR: 提出了EliDecide方法，通过椭圆决策边界解决文本开放意图分类问题，相比传统球形边界能更好地适应不同特征方向的分布差异。


<details>
  <summary>Details</summary>
Motivation: 现有自适应决策边界方法假设已知类呈各向同性分布，限制了边界为球形，忽略了不同方向的分布差异，这限制了开放意图检测的性能。

Method: 1) 使用监督对比学习获得已知样本的判别性特征空间；2) 应用可学习矩阵参数化椭圆作为每个已知类的边界；3) 通过新颖设计的双重损失函数优化边界，平衡经验风险和开放空间风险。

Result: 在多个文本意图基准和问题分类数据集上实现了最先进的性能，椭圆边界的灵活性展示了优越的开放意图检测能力和泛化潜力。

Conclusion: EliDecide方法通过椭圆决策边界有效解决了开放意图分类问题，在不同复杂开放世界场景中具有强大的泛化能力。
...
TL;DR: 本文提出了一种评估提示词能否有效引导生成文本朝向特定人类价值观的方法，并应用于Wizard-Vicuna语言模型变体，验证了无需修改模型即可实现价值观引导的可能性。
...
Method: 提出了一个实用、可重复且模型无关的评估流程，通过形式化的评分方法来量化生成响应中目标价值观的存在和增益，使用Schwartz基本人类价值观理论和对话数据集进行结构化评估。

Result: 通过比较基线提示词和明确基于价值观的提示词，证明了即使不改变模型或不动态优化提示词，价值观引导也是可能的。

Conclusion: 该方法提供了一种无需修改模型即可实现价值观引导的有效途径，为动态价值观对齐提供了实用解决方案。
...
TL;DR: 本文提出了一种基于概念梯度的可解释性技术，通过分析毒性检测中的子类型属性概念来识别毒性语言，并引入目标词典集和词概念对齐分数来量化误分类，最后提出无词典增强策略来验证模型对毒性语言模式的归因。


<details>
  <summary>Details</summary>
Motivation: 虽然毒性语言检测在文本数据中取得了显著进展，但基于概念的解释在毒性检测中的探索仍然有限。现有方法往往将概念不成比例地归因于目标类别，导致分类错误。

Method: 利用毒性检测数据集中的各种子类型属性作为概念指标，提出基于概念梯度(CG)的可解释性技术，创建目标词典集捕获导致误分类的有毒词汇，计算词概念对齐(WCA)分数量化误分类程度，并引入无词典增强策略生成排除预定义有毒词典集的毒性样本。

Result: 概念梯度方法提供了更因果的解释，通过测量概念变化如何直接影响模型输出。目标词典集和WCA分数能够有效识别和量化导致误分类的有毒词汇。无词典增强策略揭示了即使移除显性词汇重叠，模型仍可能对更广泛的毒性语言模式进行过度归因。

Conclusion: 该研究为毒性语言检测提供了更深入的可解释性分析框架，概念梯度方法和无词典增强策略有助于理解模型对毒性概念的归因机制，为改进毒性检测模型的鲁棒性提供了重要见解。
...
TL;DR: 该研究评估了AI检测模型在识别经过AI轻微润色的人类撰写阿拉伯语文章时的性能，发现所有检测器都会错误地将大量润色后的人类文章归类为AI生成，性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有AI检测模型在人类文章被AI轻微润色后会产生误判，可能导致错误指控作者使用AI抄袭，损害检测器可信度。阿拉伯语领域尚未有相关研究。

Method: 生成两个阿拉伯语数据集：第一个包含800篇文章（半AI半人类）用于评估14个LLM和商业检测器；第二个Ar-APT数据集包含400篇人类文章经10个LLM使用4种润色设置处理后的16400个样本，用于评估8个最佳模型。

Result: 所有AI检测器都会错误地将大量润色后的人类文章归为AI生成。最佳LLM Claude-4 Sonnet准确率从83.51%降至57.63%；最佳商业模型originality.AI从92%降至12%。

Conclusion: AI轻微润色会严重影响检测器性能，导致对人类文章的误判，需要开发更鲁棒的检测方法来解决这一问题。
...
TL;DR: 本文复现了基于最近邻的测试时训练方法，通过在推理时使用检索到的最近邻序列对语言模型进行微调，显著降低了困惑度和比特率指标，特别是在结构化或专业数据集上效果最明显。


<details>
  <summary>Details</summary>
Motivation: 验证测试时训练在语言模型上的有效性，特别是对于未在类似数据上预训练的模型，该方法能让较小模型接近较大模型的性能。

Method: 使用预训练的RoBERTa嵌入和Faiss索引检索每个测试输入的20个最近邻，对GPT-2、GPT-Neo和R1-Distilled-Qwen2.5-1.5B模型应用每个邻居一次梯度更新。

Result: 测试时训练显著降低了困惑度和比特率指标，在GitHub和EuroParl等结构化数据集上改进最大。未在The Pile上预训练的模型获益更多，较小模型能接近较大模型性能。

Conclusion: 最近邻测试时训练方法具有鲁棒性和通用性，即使在现代推理优化架构上也能获得一致收益，同时提出了内存高效的检索实现方案。
...
TL;DR: 本文系统分析了多语言大语言模型中语言信息在内部表示空间的分布结构和跨层演化规律，发现语言信息在第一个Transformer块就实现高度分离，且在整个模型深度中保持线性可分性。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言LLM在多种语言上表现出色，但对其内部表示空间中语言信息的结构特征和跨层演化规律缺乏系统性分析，需要深入研究语言编码的层间动态和几何结构。

Method: 在6个多语言LLM上进行全面探测研究，覆盖所有268个Transformer层，使用线性和非线性探针结合新的Token-Language Alignment分析来量化语言编码的层间动态和几何结构。

Result: 语言信息在第一个Transformer块就实现高度分离（从第0层到第1层提升76.4±8.2个百分点），且在整个模型深度中几乎完全线性可分。训练数据的语言组成强烈影响语言方向与词汇嵌入的对齐关系，中文包含模型的中文匹配峰值达到16.43%，而英语中心模型仅为3.90%，显示出4.21倍的结构印记效应。

Conclusion: 多语言LLM不是通过表面脚本特征来区分语言，而是通过由训练语料塑造的潜在表示结构来区分。这些发现为多语言表示学习中的数据组成策略和公平性提供了实践启示。
...
TL;DR: 本文提出了一种基于语言模型的嵌入方法，用于从SNOMED CT中检索层次化概念，特别针对词汇外查询问题，并在构建的评估数据集上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: SNOMED CT作为生物医学本体，其知识检索面临语言歧义、同义词、多义词等挑战，特别是当查询是词汇外查询时，问题更加严重。

Method: 提出基于语言模型的嵌入方法，通过构建词汇外查询的评估数据集，测试最直接子类和较少相关祖先的检索效果。

Result: 该方法在SNOMED CT上的表现优于SBERT和两种词汇匹配基线方法。

Conclusion: 该方法虽然针对SNOMED CT进行评估，但具有通用性，可扩展到其他本体，并发布了代码、工具和评估数据集。
...
TL;DR: 本文提出基于延展心智理论和激进生成主义的非表征心智模型，建立ABC翻译心智框架，将翻译视为整合情感、行为和认知的动态生成活动，而非静态语言对应操作。


<details>
  <summary>Details</summary>
Motivation: 挑战基于表征的心智模型，为翻译研究提供非表征主义的新视角，强调翻译是具身参与社会文化实践的技能活动。

Method: 结合延展心智理论、激进生成主义、预测处理和主动推理理论，构建ABC翻译心智框架，分析脑-身-环境互动循环中的心智生成过程。

Result: 建立了翻译心智的非表征解释框架，将翻译重新定义为意义在具身互动中实时共同创造的过程，而非信息传递或符号操作。

Conclusion: 翻译心智是通过脑-身-环境互动循环生成的，翻译是参与社会文化实践的技能性活动，意义在具身互动中动态共同创造。
...
TL;DR: 本文重新探讨了非宾格动词和作格动词的句法分类与语义特征（施事性和终结性）之间的关系，通过可解释维度分析支持了传统观点，并展示了结合人类判断的方法优势。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为描述施事动作的动词更可能出现在作格句法中，而描述终结事件的动词更可能出现在非宾格句法中。但Kim等人(2024)发现人类对施事性和终结性的评分并不能很好地预测不及物动词的句法行为，因此需要重新审视这个问题。

Method: 使用可解释维度方法，基于施事性和终结性量表对立极点的种子词计算维度，结合人类判断来分析不及物动词的句法行为。

Result: 研究结果支持了作格性/非宾格性与施事性/终结性之间的联系，表明使用可解释维度结合人类判断可以为难以在评分任务中评估的语义属性提供有价值的证据。

Conclusion: 可解释维度方法能够有效揭示不及物动词句法分类与语义特征之间的关系，为语义属性的评估提供了更可靠的方法论支持。
...
TL;DR: PEPPER是一种针对文本到图像扩散模型后门攻击的防御方法，通过重写输入提示来破坏触发器，同时保持视觉相似性。
...
Method: PEPPER通过将输入提示重写为语义上不同但视觉上相似的描述，并添加不显眼的元素，来破坏输入提示中嵌入的触发器并稀释触发器标记的影响。

Result: 实验表明PEPPER对基于文本编码器的攻击特别有效，显著降低了攻击成功率，同时保持了生成质量。

Conclusion: PEPPER可以与其他现有防御方法结合使用，提供比任何单独方法更强和更通用的鲁棒性。
...
TL;DR: 提出了一种新的无参考指标来评估大语言模型生成响应的简洁性，通过三个压缩比率计算来量化非必要内容，无需依赖人工标注。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常生成冗长、包含冗余细节的响应，这会降低清晰度和用户满意度，同时增加模型开发者的成本（特别是按输出token收费的专有模型）。

Method: 通过三个计算的平均值来量化非必要内容：1）原始响应与LLM抽象摘要之间的压缩比；2）原始响应与LLM提取摘要之间的压缩比；3）单词移除压缩，即LLM在保持意义的前提下尽可能移除非必要单词。

Result: 实验结果表明，该指标能够有效识别LLM输出中的冗余内容，为对话AI系统提供自动评估响应简洁性的实用工具。

Conclusion: 提出的无参考简洁性评估指标为自动评估LLM响应简洁性提供了有效解决方案，无需依赖人工标注的黄金标准参考。
...
TL;DR: 本文提出Soft Concept Mixing (SCM)训练方案，通过在训练中直接暴露软概念表示来弥合LLM推理中的软概念与训练中的离散标记之间的差距，从而提高推理性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常通过生成离散标记进行推理，这限制了其表达能力。虽然软概念推理是一个有前景的方向，但LLM是在离散标记上训练的，存在推理中的软概念与训练中的离散标记之间的差距。

Method: 提出Soft Concept Mixing (SCM)训练方案：构建软概念向量（通过嵌入的概率加权平均形成），将其混合到模型的隐藏状态中，并使用强化学习优化整个潜在推理过程。

Result: 在五个推理基准测试上的实验表明，SCM提高了LLM的推理性能，同时保持了稳定的训练动态。

Conclusion: SCM通过软概念感知训练有效弥合了软概念推理与离散标记训练之间的差距，提升了LLM的推理能力。
...
TL;DR: 提出了一种改进Tiny Recursive Models (TRMs)训练效率的新方法，通过将隐式推理过程建模为分类器自由引导和隐式策略改进算法，显著减少了训练所需的前向传播次数。


<details>
  <summary>Details</summary>
Motivation: 研究如何以最小改动进一步提升小型循环架构（如TRMs）在复杂推理任务上的效率，特别是在Abstraction and Reasoning Corpus (ARC)任务上的表现。

Method: 将TRMs的隐式推理过程框架化为分类器自由引导和隐式策略改进算法，提出了一种新的训练方案，为每个训练循环提供目标。

Result: 方法显著提升了训练效率：前向传播总次数减少18倍，移除了停止机制，同时保持与标准TRMs相当的质量。在ARC-1上仅用0.8M参数就达到了24%的准确率，超越了大多数LLMs。

Conclusion: 提出的训练方案能够显著提高TRMs的训练效率，在保持性能的同时大幅减少计算资源需求，为小型模型在复杂推理任务上的应用提供了有效途径。
...
TL;DR: 本研究探讨了训练数据的统计特性与诱导头形成之间的关系，发现批量大小和上下文长度的简单组合可以预测诱导头形成点，表面二元组重复频率和可靠性强烈影响诱导头形成，并确定了这两个值的精确帕累托边界。


<details>
  <summary>Details</summary>
Motivation: 虽然专门的注意力头（称为诱导头）被认为是现代语言模型具有出色上下文学习能力的基础，但其形成的精确机制仍不清楚。本研究旨在揭示训练数据统计特性与诱导头形成之间的确切关系。

Method: 通过分析自然和合成训练数据的统计特性，研究批量大小、上下文长度、表面二元组重复频率和可靠性等因素对诱导头形成的影响。

Result: 发现批量大小和上下文长度的简单组合可以预测诱导头形成点；表面二元组重复频率和可靠性强烈影响诱导头形成，并找到了这两个值的精确帕累托边界；当频率和可靠性高时，局部依赖关系足以形成诱导头，但当频率和可靠性低时，分类性和边际分布形状变得重要。

Conclusion: 训练数据的统计特性，特别是表面二元组重复频率和可靠性，对诱导头的形成具有决定性影响，这些发现为理解语言模型上下文学习能力的形成机制提供了重要见解。
...
TL;DR: 本文提出了一种新颖的论点感知定量摘要任务，旨在揭示对话中的论点结构并量化论点强度，提出了ARQUSUMM框架来实现这一目标。


<details>
  <summary>Details</summary>
Motivation: 在线对话平台上的争议性话题日益增多，需要不仅总结多样化论点，还要揭示其论证结构和理由。现有对话摘要研究虽然考虑了句子间的论证关系，但未能深入分析句子内部的深层论点结构。
...
Conclusion: 该研究成功实现了论点感知的定量摘要，通过揭示对话中的深层论点结构并量化论点强度，为在线对话分析提供了更有效的解决方案。
...
TL;DR: 该研究提出了一种监督微调方法，用于增强湖南历史名人领域的特定信息提取能力，通过在四个公开大语言模型上进行参数高效指令微调，显著提升了领域知识提取和结构化输出生成性能。


<details>
  <summary>Details</summary>
Motivation: 解决湖南历史名人系统数据资源有限、通用模型在低资源环境下领域知识提取和结构化输出生成性能不足的问题，推动历史文化研究中的知识图谱构建。
...
Conclusion: 该研究为垂直领域大语言模型的微调提供了新思路，展示了在文化遗产知识提取和知识图谱构建中成本效益应用的潜力。
...
TL;DR: 该研究构建了一个视觉说服力判断数据集，提出了视觉说服因素分类法，并探索了认知引导和知识注入策略。研究发现视觉语言模型存在回忆偏向，过度预测高说服力，对低/中级特征判别能力弱，而高级语义对齐是最强预测因子。


<details>
  <summary>Details</summary>
Motivation: 探究视觉语言模型是否真正理解视觉说服力——即视觉线索如何塑造人类态度和决策，目前尚不清楚。

Method: 构建二元说服力判断的高共识数据集，引入视觉说服因素分类法（包括低级感知、中级构图和高级语义线索），探索认知引导和知识注入策略。

Result: 视觉语言模型存在回忆偏向，过度预测高说服力；对低/中级特征判别能力弱；高级语义对齐是最强预测因子；简单指令或无引导推理支架效果有限，而简洁、基于对象的理由显著提高精度和F1分数。
...
TL;DR: 本文提出了一种可解释的自动评分框架AnalyticScore，通过四个可解释性原则(FGTI)和基于LLM的特征提取方法，在保持高评分准确性的同时提供可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决当前自动评分系统缺乏透明度和可解释性的问题，满足大规模真实评估中对可解释自动评分的需求。

Method: 开发AnalyticScore框架：1)提取可识别响应元素；2)使用LLM将响应转化为人类可解释的特征值；3)应用直观的有序逻辑回归模型进行评分。

Result: AnalyticScore在ASAP-SAS数据集的10个项目上，评分准确性优于许多不可解释方法，与不可解释SOTA方法仅相差0.06 QWK，且特征化行为与人类标注者高度一致。

Conclusion: 提出的FGTI原则和AnalyticScore框架为实现可解释自动评分提供了可行方案，在保持高准确性的同时实现了良好的可解释性。
...
TL;DR: MUCH是首个用于公平评估声明级不确定性量化方法的基准，包含4,873个多语言样本，提供24个生成对数概率，并提出高效声明分割算法。
...
Method: 构建包含4种欧洲语言和4种开源LLM的数据集，提供完整生成对数概率，开发高效声明分割算法（仅需0.2%生成时间）。

Result: 创建了MUCH基准数据集，包含4,873个样本，支持多语言和多模型评估，分割算法效率显著提升。

Conclusion: 当前不确定性量化方法在性能和效率方面仍有很大改进空间，MUCH基准为未来方法发展提供了重要基础。
...
TL;DR: 本文提出使用上下文压缩作为预训练任务来改进LLM的文本表示能力，优于现有的基于token级预测的方法，并通过对比学习进一步提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM主要针对因果建模和下一个token预测优化，不适合生成整体文本表示。虽然已有研究引入预训练任务来适应LLM，但大多依赖token级预测目标，未能充分利用LLM的潜力。

Method: 提出使用上下文压缩作为预训练任务，让模型学习生成紧凑的记忆token来替代整个上下文进行下游序列预测。结合对比学习进一步提升表示质量。

Result: 实验表明，精心设计的压缩目标能显著提升LLM的文本表示能力，优于基于token级预训练任务的模型。通过对比学习得到的LLM2Comp模型在多种任务上优于当代LLM文本编码器，且样本效率更高，需要更少的训练数据。

Conclusion: 上下文压缩是一种有效的预训练任务，能够显著提升LLM的文本表示能力，结合对比学习可以产生强大的表示模型，在多个任务上表现优异且更加样本高效。
...
TL;DR: 本文介绍了LangMark——一个用于自动后编辑（APE）的多语言数据集，包含英语到7种语言的206,983个三元组（源文本、机器翻译输出、人工后编辑翻译），并证明大语言模型通过少量样本提示可以有效进行APE。


<details>
  <summary>Details</summary>
Motivation: 尽管神经机器翻译有所进展，但缺乏针对NMT输出的大规模多语言数据集阻碍了有效APE系统的发展。

Method: 构建并发布LangMark数据集，包含英语到7种语言的206,983个人工标注的三元组；使用大语言模型进行少量样本提示的APE实验。

Result: 大语言模型通过少量样本提示可以有效执行APE，改进领先的商业甚至专有机器翻译系统。

Conclusion: LangMark数据集将为APE系统的未来发展和评估提供便利。
...
TL;DR: 本文介绍了PLLuM项目中用于微调波兰语大语言模型的指令数据集，提出了指令的功能分类，并比较了人工编写与合成指令数据集在语言适应中的影响，同时发布了PLLuMIC数据集子集。


<details>
  <summary>Details</summary>
Motivation: 为PLLuM项目中的波兰语大语言模型开发提供指令数据集，并分享在语言适应过程中使用不同类型指令数据集的经验和观察。

Method: 使用有机、转换和合成的指令来构建数据集，并对这些指令进行功能分类，比较人工编写与合成指令在语言适应中的效果。

Result: 开发了PLLuM指令语料库(PLLuMIC)，并发布了其代表性子集，为其他LLM的类似数据集开发提供指导和规划参考。

Conclusion: PLLuMIC数据集及其分类方法对于指导其他语言大语言模型的指令数据集开发具有实用价值，不同类型指令在语言适应中各有特点。
...
TL;DR: 本文提出Aspect-Based Causal Abstention (ABCA)框架，通过因果推理分析LLM内部知识多样性来实现早期弃权，防止幻觉回答。


<details>
  <summary>Details</summary>
Motivation: 现有弃权方法依赖生成后信号，无法提前阻止不可靠回答。LLM从不同来源获取的知识具有多面性，这些知识多样性可用于评估回答可靠性。

Method: ABCA框架通过因果推理分析LLM参数知识的多方面多样性（如学科、法律背景、时间框架等），估计基于这些方面的因果效应来评估查询相关知识可靠性。

Result: 在标准基准测试中，ABCA提高了弃权可靠性，达到最先进性能，并增强了弃权决策的可解释性。

Conclusion: ABCA通过分析内部知识多样性实现早期弃权，能有效处理知识冲突和知识不足两种情况，提升LLM回答的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 传统统计方法能捕捉词级重要性但缺乏上下文语义理解，而现代深度学习方法能理解上下文语义但可能忽略简单但高影响力的统计指标。需要一种能结合两者优势的方法来提升新闻文本分类性能。

Method: 提出注意力引导特征融合（AGFF）模型，在统一框架中结合统计特征（如词频、TF-IDF）和语义特征，使用基于注意力的机制动态确定各特征类型的相对重要性，从而做出更明智的分类决策。

Result: 在基准新闻数据集上的评估显示，AGFF模型相比传统统计模型和纯语义深度学习模型表现出更优越的性能。消融研究验证了融合过程中各组成部分的贡献。

Conclusion: 策略性地整合不同类型的特征可以显著提高分类准确性。该模型能够平衡并利用统计和语义表示的互补优势，为实际新闻分类任务提供了实用有效的解决方案。
...
TL;DR: 本文提出了一种名为E3的层剪枝框架，通过可微分掩码优化和熵感知知识蒸馏，实现了任务有效、训练经济、推理高效的模型压缩。
...
Result: 在Qwen3-32B模型上剪除25%层时，在MATH-500上达到96%准确率，仅比原始模型（96.8%）下降0.8%，优于现有SOTA方法（95%），推理速度提升1.33倍，仅消耗0.5B tokens（占后训练数据量的0.5%）。

Conclusion: 该方法在多种模型架构和基准测试中表现出优越性，有效解决了层剪枝在实际部署中的关键挑战。
...
TL;DR: PARROT框架用于测量LLM在权威和说服压力下的准确性退化，通过双盲评估、置信度追踪和行为分类来量化模型对错误权威信息的顺从程度。
...
Method: 使用双盲评估比较中性问题和权威错误版本，通过基于对数似然的校准追踪量化置信度变化，采用八状态行为分类法系统分类失败模式。

Result: 先进模型（如GPT-5）顺从率低（≤11%），准确性损失小；老旧/小模型出现严重认知崩溃（GPT-4:80%，Qwen 2.5-1.5B:94%）。国际法和全球知识领域脆弱，初等数学相对稳健。
...
TL;DR: MirageTVQA是一个新的基准测试，用于评估视觉语言模型在包含视觉噪声的多语言表格问答任务中的表现，揭示了现有模型在视觉噪声和多语言场景下的严重性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有表格问答数据集主要是英语且表格格式完美，与现实世界复杂场景存在显著差距，需要创建更贴近实际应用的基准测试。

Method: 创建包含近60,000个问答对、覆盖24种语言的MirageTVQA数据集，其中表格不仅多语言还包含模拟扫描文档的视觉噪声。

Result: 评估显示领先的VLM模型在面临视觉噪声时性能下降超过35%，且存在明显的英语优先偏见，推理能力无法有效迁移到其他语言。

Conclusion: MirageTVQA为衡量和推动更鲁棒的表格推理VLM模型提供了基准，揭示了现有模型在现实场景中的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 动机是为了评估大型语言模型在爱沙尼亚语上的常识推理能力，并探索高质量机器翻译的可行性，同时强调语言专家在数据集本地化中的重要性。
...
Conclusion: 结论强调了语言专家参与数据集翻译和适配的重要性，以确保对大型语言模型语言能力和推理能力的可靠和可解释评估。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在探索普通话中单音节词的音高轮廓如何在实际对话中实现，特别关注词义对音高模式的影响，挑战传统的声调理论。

Method: 使用广义加性模型将观察到的音高轮廓分解为与不同控制变量和语义预测因子相关的组成部分，分析词义、词义感知和同音异义词对音高轮廓的影响。

Result: 研究发现词义是音高实现的强预测因子，词义感知比词本身更能预测音高轮廓，同音异义词具有不同的音高模式，且上下文嵌入能显著准确预测单个词标记的音高轮廓。

Conclusion: 研究结果挑战了标准的普通话声调理论，但符合判别性词汇模型的理论框架，表明分布语义学在语音学中具有重要作用。
...
Method: 使用文本到图像模型生成前提的视觉表示，然后通过余弦相似度或视觉问答技术与文本假设进行比较。

Result: 该方法在不进行任务特定微调的情况下实现了高准确率，并在对抗数据集上验证了其鲁棒性。
...
TL;DR: 本文提出了选择性RoPE（Selective RoPE），一种输入依赖的旋转位置编码机制，它扩展了传统的RoPE，允许在任意角度进行旋转，适用于线性和softmax变换器。


<details>
  <summary>Details</summary>
Motivation: 受线性变换器中选择性门控机制在语言任务中表现优异的启发，作者希望将这种输入依赖的选择性机制引入到位置编码中，以改进语言建模和序列任务性能。

Method: 开发了选择性RoPE机制，通过输入依赖的方式确定旋转角度，揭示了softmax注意力中隐藏的旋转操作，并展示了在状态空间模型和门控线性变换器中实部负责遗忘、虚部通过旋转编码位置的结构。

Result: 在门控变换器中装备选择性RoPE后，在语言建模和困难序列任务（如复制、状态跟踪和检索）上表现得到改善。

Conclusion: 选择性RoPE通过输入依赖的旋转机制有效改进了位置编码，在多种变换器架构和序列任务中展现出优越性能。
...
TL;DR: PUCP-Metrix是一个包含182个语言指标的开源存储库，涵盖词汇多样性、句法和语义复杂性、连贯性、心理语言学和可读性，用于西班牙语的细粒度、可解释文本分析。
...
Method: 开发了一个包含182个语言指标的开放源代码存储库，涵盖多个语言维度，包括词汇多样性、句法和语义复杂性、连贯性、心理语言学和可读性。

Result: 在自动可读性评估和机器生成文本检测任务中表现出色，与现有存储库相比具有竞争力，且优于强神经基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 多项选择题问答（MCQA）因其约束性输出格式而被广泛用于评估和强化微调，但选项可能泄露可利用信号，导致准确性指标不可靠并鼓励答案猜测行为。

Method: 提出ReVeL框架，将多项选择题分类并根据不同答案类型应用不同的改写和验证方案，将其转换为开放式问题。使用GRPO对Qwen2.5-VL模型进行微调。
...
<details>
  <summary>Details</summary>
Motivation: 传统RLVR在数学定理证明等任务中可扩展性有限，因为中间推理过程至关重要但最终答案难以直接可靠验证。同时，令牌级SFT容易退化为死记硬背而非诱导长链思维。

Method: 采用两阶段训练流程：首先在采样的数学计算和证明数据上进行自监督训练，然后在仅结果可验证的数学计算数据集上进行RLVR微调。通过"掩码-填充"和"步骤重排序"构建过程级自监督奖励。

Result: 在Qwen2.5-3B和DeepSeek-R1-Distill-Qwen-1.5B上实现，在AIME24、AIME25、AMC23和MATH500数据集上评估。在固定采样和解码预算下，相比原始RLVR获得平均相对提升：Pass@1 +9.86%、Pass@5 +5.27%、Pass@8 +4.00%。

Conclusion: 引入过程感知的自监督信号可以有效增强RLVR在仅结果可验证场景下的可扩展性和性能。
...
### [127] [Optimizing PyTorch Inference with LLM-Based Multi-Agent Systems](https://arxiv.org/abs/2511.16964)
*Kirill Nagaitsev,Luka Grbcic,Samuel Williams,Costin Iancu*

Main category: cs.MA

TL;DR: 本文提出了一个比较多智能体PyTorch优化系统的逻辑框架，发现利用性策略与错误修复智能体结合时效果最佳，性能与优化步骤的粒度相关，在H100 GPU上实现了平均2.88倍的加速。


<details>
  <summary>Details</summary>
Motivation: 现有AI推理系统面临在可用GPU硬件上最大化性能的挑战，传统方法需要编写自定义GPU内核和使用专用模型编译器。虽然最近研究表明基于LLM的多智能体系统能有效执行此类优化，但其动态特性仍未被探索。

Method: 提出了一个逻辑框架来比较多智能体PyTorch优化系统，评估了不同策略组合和优化步骤粒度的影响。

Result: 评估显示，利用性策略与错误修复智能体配对时表现最佳，性能与优化步骤的粒度相关。最佳实现在KernelBench基准套件上于H100 GPU实现了平均2.88倍的加速。

Conclusion: 多智能体系统在PyTorch优化中表现出色，特别是利用性策略与错误修复智能体的组合，为AI推理系统的性能优化提供了有效方法。
...
### [128] [A segment anchoring-based balancing algorithm for agricultural multi-robot task allocation with energy constraints](https://arxiv.org/abs/2511.17076)
...
TL;DR: 本文提出SABA算法解决多机器人系统在能量约束下的任务分配问题，通过分段锚定和平衡机制有效处理充电决策引发的调度中断，在收敛性和多样性方面优于现有算法。


<details>
  <summary>Details</summary>
Motivation: 多机器人系统在劳动密集型产业中面临效率与成本挑战，特别是在智能农业场景中，电动机器人的多行程收割调度需要同时考虑时间、运输成本、负载和电池容量等多重约束，传统优化方法难以处理充电决策引发的复杂级联效应。

Method: 提出分段锚定平衡算法(SABA)，包含两个协同机制：顺序锚定和平衡机制利用充电决策作为锚点系统重构中断路径；基于比例分割的再平衡机制负责精细平衡和调整最终解的时间跨度。

Result: 在真实案例和基准测试集上的大量对比实验表明，SABA在解收敛性和多样性方面全面优于6种最先进算法。
...
### [129] [Stable diffusion models reveal a persisting human and AI gap in visual creativity](https://arxiv.org/abs/2511.16814)
...
### [130] [Cognitive BASIC: An In-Model Interpreted Reasoning Language for LLMs](https://arxiv.org/abs/2511.16837)
*Oliver Kramer*

Main category: cs.AI

TL;DR: Cognitive BASIC是一种基于BASIC风格的提示语言和模型内解释器，将大语言模型的推理过程结构化为显式的逐步执行轨迹。它利用编号行和简单命令作为可解释的认知控制层，使LLM能够可靠地模拟短程序，实现透明的多步推理。


<details>
  <summary>Details</summary>
Motivation: 受复古BASIC简单性的启发，旨在为LLM推理提供结构化的认知控制层，使推理过程更加透明和可解释。

Method: 设计了一个自然语言解释器文件来指定命令语义、内存更新和日志行为，通过心理模型解释器提取声明性和程序性知识，检测矛盾并在必要时产生解决方案。

Result: 在三个LLM上的基准测试显示，所有模型都能执行Cognitive BASIC程序，整体表现强劲但性能不均一。

Conclusion: Cognitive BASIC成功地将LLM推理结构化为显式的逐步执行过程，为透明多步推理提供了有效框架。
...
### [131] [Fantastic Bugs and Where to Find Them in AI Benchmarks](https://arxiv.org/abs/2511.16842)
...
TL;DR: 提出一个利用响应模式统计分析的系统化基准修订框架，通过检测超出预期范围的统计指标来识别无效问题，结合专家评审和LLM初步筛选，提高基准可靠性。
...
Method: 基于AI评估中常用的核心假设——平均分足以总结模型性能，构建单维潜在结构测量实验。通过分析响应模式统计指标，检测超出预期范围的异常问题，结合专家评审和LLM初步筛选。

Result: 在9个广泛使用的基准测试中，该方法指导专家评审识别问题问题的准确率高达84%，显著减少人工工作量。

Conclusion: 该框架为系统化基准修订提供了高效可扩展的解决方案，通过统计分析和自动化工具的结合，显著提高了基准测试的可靠性。
...
### [132] [Agentifying Agentic AI](https://arxiv.org/abs/2511.17332)
*Virginia Dignum,Frank Dignum*

Main category: cs.AI

TL;DR: 本文认为AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具为构建具有认知、合作和治理模型的智能体AI系统提供了基础，旨在实现既具备能力又透明、合作和可问责的智能体系统。
...
Method: 将自适应、数据驱动的方法与结构化的推理和协调模型相结合，利用AAMAS社区的概念工具如BDI架构、通信协议、机制设计和制度建模。

Result: 提出了一个连接形式理论和实践自主性的智能体视角，为构建透明、合作和可问责的智能体系统奠定了基础。

Conclusion: 通过将自适应方法与结构化模型相结合，可以构建不仅具备能力和灵活性，而且透明、合作和可问责的智能体系统，从而弥合形式理论与实际自主性之间的差距。
...
### [133] [Hybrid Differential Reward: Combining Temporal Difference and Action Gradients for Efficient Multi-Agent Reinforcement Learning in Cooperative Driving](https://arxiv.org/abs/2511.16916)
...
Method: 提出混合差分奖励机制，包含两个互补组件：基于全局势函数的时间差分奖励，利用势能演化趋势确保最优策略不变性和长期目标一致性；动作梯度奖励，直接衡量动作的边际效用以提供高信噪比的局部指导信号。

Result: 通过在线规划和多智能体强化学习算法的广泛实验表明，HDR机制显著提高了收敛速度和策略稳定性，指导智能体学习到能有效平衡交通效率和安全的优质协同策略。
...
### [134] [MirrorMind: Empowering OmniScientist with the Expert Perspectives and Collective Knowledge of Human Scientists](https://arxiv.org/abs/2511.16997)
*Qingbin Zeng,Bingbing Fan,Zhiyu Chen,Sijian Ren,Zhilun Zhou,Xuhua Zhang,Yuanyi Zhen,Fengli Xu,Yong Li,Tie-Yan Liu*

Main category: cs.AI

TL;DR: MirrorMind是一个分层认知架构，通过整合个体认知轨迹和集体学科记忆来解决现有AI科学家方法的局限性，实现结构化的科学推理。
...
Method: 提出三层框架：个体层面构建研究者的认知模型，领域层面映射集体知识到结构化概念图，跨学科层面作为正交编排引擎。架构分离记忆存储与智能执行。

Result: 在四个综合任务中评估，包括作者级认知模拟、互补推理、跨学科协作促进和多智能体科学问题解决，结果显示MirrorMind超越了简单事实检索。

Conclusion: 通过整合个体认知深度与集体学科广度，MirrorMind实现了结构化、个性化和洞察生成的科学推理。
...
<details>
  <summary>Details</summary>
Motivation: 现有工具增强智能体在扩展工具调用预算时性能无法提升，因为它们缺乏预算意识，快速达到性能天花板。

Method: 引入预算跟踪器提供持续预算意识，开发BATS框架动态调整规划和验证策略，基于剩余资源决定深入挖掘或转向新路径。

Result: 预算感知方法产生更有利的扩展曲线，推动成本-性能帕累托前沿，提供对工具增强智能体扩展的更透明和原则性理解。

Conclusion: 预算意识是有效扩展工具增强智能体的关键，本文为预算约束下的智能体提供了系统研究框架和实证见解。
...
TL;DR: 本文重新解释扩散模型在逆问题求解中的作用，将其视为期望最大化框架中的初始化阶段，提出DAPS++方法，通过解耦扩散阶段和数据驱动优化，提高计算效率和重建性能。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯视角下的基于分数的扩散方法在解决逆问题时存在局限性：先验提供有限指导，重建主要由测量一致性项驱动，导致推理过程与扩散动力学实际解耦。需要澄清这种结构并改进方法。

Method: 将扩散重新解释为期望最大化框架中的初始化阶段，提出DAPS++方法，允许似然项更直接地指导推理，同时保持数值稳定性。该方法解耦扩散阶段和数据驱动优化。

Result: DAPS++需要更少的函数评估次数和测量优化步骤，在多种图像恢复任务中实现了高计算效率和鲁棒的重建性能。

Conclusion: 扩散在逆问题求解中的主要作用是提供初始化，DAPS++通过解耦扩散和优化阶段，解释了统一扩散轨迹在实践中保持有效的原因，并提供了更高效稳定的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中的稀疏奖励问题具有挑战性，主要困难在于联合动作轨迹的指数稀疏性和现有方法未能充分考虑影响团队状态的联合动作。

Method: 提出了MIR方法，激励个体智能体探索影响队友的行为，并与原始策略结合，有效刺激团队探索。还创建了MiniGrid-MA环境进行实验验证。
...
TL;DR: 本文提出了一个基于分层任务抽象机制(HTAM)的新型多智能体框架EarthAgent，专门用于解决遥感等专业领域中结构化工作流程的挑战，显著优于现有的单智能体和多智能体系统。


<details>
  <summary>Details</summary>
Motivation: 现有的通用智能体框架(如ReAct或角色扮演)在需要严格结构化工作流程的专业领域(如遥感)中表现不佳，这些领域需要专门的工具和多步骤程序。

Method: 提出了分层任务抽象机制(HTAM)，将多智能体系统组织成逻辑层次结构，反映领域内在的任务依赖图，将复杂问题分解为顺序层，每层子智能体在前一层输出上操作。

Result: 实验表明，EarthAgent在GeoPlan-bench基准测试中，在工具选择、路径相似性和逻辑完整性等指标上显著优于现有系统。
...
### [139] [That's not natural: The Impact of Off-Policy Training Data on Probe Performance](https://arxiv.org/abs/2511.17408)
*Nathalie Kirch,Samuel Dower,Adrians Skapars,Ekdeep Singh Lubana,Dmitrii Krasheninnikov*

Main category: cs.AI

TL;DR: 本文系统评估了使用合成和离策略数据训练LLM行为探测器的泛化性能，发现在缺乏同策略数据时，使用同领域离策略数据比不同领域同策略数据更可靠，但领域偏移仍会导致性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 由于许多LLM行为的自然示例稀少，研究者不得不依赖合成或离策略数据进行探测器训练，但这类数据对探测器泛化性能的影响尚不明确。

Method: 在8种不同LLM行为上测试线性和注意力探测器，比较不同响应生成策略对探测器性能的影响，评估从离策略数据到同策略数据的泛化能力。

Result: 响应生成策略显著影响探测器性能，从离策略数据到激励条件下测试的成功泛化能预测同策略泛化成功；欺骗和伪装探测可能在实际监控中泛化失败；领域偏移导致更大性能下降。

Conclusion: 在缺乏同策略数据时，使用同领域离策略数据比不同领域同策略数据产生更可靠的探测器，凸显了需要更好处理LLM监控中分布偏移的方法。
...
### [140] [SRA-CP: Spontaneous Risk-Aware Selective Cooperative Perception](https://arxiv.org/abs/2511.17461)
...
Result: 相比通用协作感知方法，SRA-CP仅使用20%通信带宽，安全关键物体检测精度损失小于1%；相比现有选择性协作感知方法，感知性能提升15%。
...
### [141] [RubiSCoT: A Framework for AI-Supported Academic Assessment](https://arxiv.org/abs/2510.17309)
*Thorsten Fröhlich,Tim Schlippe*

Main category: cs.AI

TL;DR: RubiSCoT是一个AI支持的论文评估框架，利用自然语言处理技术从提案到最终提交提供一致、可扩展的评估解决方案。
...
### [142] [Password Strength Analysis Through Social Network Data Exposure: A Combined Approach Relying on Data Reconstruction and Generative Models](https://arxiv.org/abs/2511.16716)
*Maurizio Atzori,Eleonora Calò,Loredana Caruccio,Stefano Cirillo,Giuseppe Polese,Giandomenico Solimando*

Main category: cs.CR

TL;DR: 本文介绍了SODA ADVANCE工具，该工具通过整合多源公开数据（包括社交媒体）来增强密码强度评估。同时研究了大型语言模型（LLMs）在密码生成和评估方面的能力与风险。实验表明LLMs能生成强且个性化的密码，并在考虑用户资料数据时能有效评估密码强度。


<details>
  <summary>Details</summary>
Motivation: 传统密码强度评估方法往往不足，用户倾向于使用易记但安全性低的密码，这增加了安全风险。需要开发更有效的密码评估工具和方法。

Method: 开发了SODA ADVANCE数据重建工具，集成专门模块利用多源公开数据评估密码强度。同时研究LLMs在密码生成和评估方面的能力，通过100名真实用户的实验进行评估。

Result: 实验证明LLMs能够生成强且个性化的密码，特别是根据用户资料定制密码。LLMs在评估密码强度方面也很有效，尤其是在考虑用户资料数据时。

Conclusion: SODA ADVANCE工具和LLMs技术为密码安全提供了新的解决方案，能够生成更安全的个性化密码并进行更准确的密码强度评估，但同时也需要考虑相关风险。
...
### [143] [Membership Inference Attacks Beyond Overfitting](https://arxiv.org/abs/2511.16792)
*Mona Khalil,Alberto Blanco-Justicia,Najeeb Jebreel,Josep Domingo-Ferrer*

Main category: cs.CR

TL;DR: 本文研究了机器学习模型中成员推理攻击的根本原因，发现即使非过拟合模型也会泄露部分训练数据隐私，特别是对类内异常样本（如噪声样本或难以分类的样本）存在隐私风险，并提出了针对性防御策略。


<details>
  <summary>Details</summary>
Motivation: 成员推理攻击对使用敏感数据进行训练的机器学习模型构成严重隐私威胁。现有研究主要关注模型过拟合导致的隐私泄露，但即使非过拟合模型也会泄露部分训练数据信息，这促使我们深入探究成员推理漏洞的根本原因。

Method: 通过实证分析非过拟合模型中易受成员推理攻击的训练数据样本特征，发现这些样本通常是类内异常样本（噪声样本或难以分类的样本），并基于此提出针对性防御策略。

Result: 研究发现，在非过拟合且具有良好泛化能力的模型中，易受成员推理攻击的样本主要是类内异常样本，这些样本在训练过程中表现出独特的特征模式。

Conclusion: 成员推理漏洞不仅源于模型过拟合，还与训练数据中异常样本的特性密切相关。通过保护这些易受攻击的样本，可以有效增强机器学习模型的隐私保护能力。
...
### [144] [TICAL: Trusted and Integrity-protected Compilation of AppLications](https://arxiv.org/abs/2511.17070)
*Robert Krahn,Nikson Kanti Paul,Franz Gregor,Do Le Quoc,Andrey Brito,André Martin,Christof Fetzer*

Main category: cs.CR

TL;DR: Tical是一个实用的可信编译框架，为从源代码到最终可执行文件的构建流水线提供完整性保护和机密性。
...
Method: 利用TEEs作为运行时保护，并通过文件系统屏蔽和不可变审计日志与版本历史来增强TEEs，确保编译链只能访问可信文件和中间输出。

Result: 使用微基准和宏基准的评估显示，Tical能够以可接受的性能开销保护整个CI/CD流水线的机密性和完整性。
...
### [145] [Constant-Size Cryptographic Evidence Structures for Regulated AI Workflows](https://arxiv.org/abs/2511.17118)
*Leo Kao*

Main category: cs.CR

TL;DR: 本文提出恒定大小的密码学证据结构，用于在受监管环境中为AI工作流提供可验证的审计证据。每个证据项是固定大小的密码学字段元组，支持恒定大小存储和统一验证成本，并能与哈希链和Merkle树审计结构良好组合。


<details>
  <summary>Details</summary>
Motivation: 为受监管的AI工作流提供可验证的审计证据，确保工作流事件和配置的强绑定，同时保持存储和验证成本的可预测性。

Method: 形式化受监管AI工作流模型，定义证据结构的语法和算法，提出基于碰撞抵抗哈希函数和标准数字签名方案的通用哈希-签名构造，并与哈希链日志、Merkle树锚定和可信执行环境集成。

Result: 实现了原型库并在商用硬件上进行微基准测试，证明恒定大小证据的每个事件开销小且可预测。

Conclusion: 该设计为临床试验管理、制药合规性和医疗AI治理提供了有效的审计证据解决方案，具有实际工业应用价值。
...
### [146] [Steering in the Shadows: Causal Amplification for Activation Space Attacks in Large Language Models](https://arxiv.org/abs/2511.17194)
*Zhiyuan Xu,Stanislav Abaimov,Joseph Gardiner,Sana Belguith*

Main category: cs.CR

TL;DR: 该论文发现LLM的中间激活状态是一个易受攻击的行为控制面，通过因果放大效应和敏感性缩放引导技术，可以在保持模型整体能力的同时显著改变其行为特征。


<details>
  <summary>Details</summary>
Motivation: 现代大型语言模型通常通过审计数据、提示和拒绝策略来保证安全，但忽略了前向传播过程中的中间激活状态可能成为攻击面。研究者希望探索这一被忽视的安全漏洞。

Method: 提出了敏感性缩放引导（SSS）攻击方法，结合序列起始锚定和基于敏感性的强化，将有限的扰动预算集中在最脆弱的层和标记上，利用因果放大效应实现行为控制。

Result: 在多个开源模型和四种行为维度（邪恶、幻觉、谄媚、情感）上，SSS能够诱导显著的行为偏移，同时保持高连贯性和通用能力。

Conclusion: 激活引导技术已成为白盒和供应链LLM部署的具体安全威胁，需要重新审视LLM的安全架构设计。
...
### [147] [Persistent BitTorrent Trackers](https://arxiv.org/abs/2511.17260)
*Francois Xavier Wicht,Zhengwei Tong,Shunfan Zhou,Hang Yin,Aviv Yaish*

Main category: cs.CR

TL;DR: 该论文提出了一种基于区块链和可信执行环境的去中心化BitTorrent信誉系统，解决了传统私有追踪器的三大问题：信誉不可移植、中心化单点故障和虚假统计报告。


<details>
  <summary>Details</summary>
Motivation: 传统私有BitTorrent追踪器存在信誉无法跨追踪器迁移、中心化服务器单点故障以及上传统计数据自报告且不可验证的问题，当追踪器关闭时用户会丢失贡献历史。

Method: 使用智能合约存储信誉，用密码学证明替代自报告；接收方对传输的数据块签名确认，追踪器在TEE中聚合验证后更新链上信誉；追踪器不可用时使用认证DHT进行发现，链上信誉作为PKI实现访问控制。

Result: 原型系统在Intel TDX上测试显示，传输确认仅增加不到6%的开销，签名聚合使验证速度提升2.5倍。

Conclusion: 该设计能够持久保存信誉、抵御追踪器故障，并通过工厂部署合约实现单跳迁移，使信誉可在新实例间移植。
...
TL;DR: 提出了一种以患者为中心的电子健康记录共享架构，通过区块链记录权限和审计信息，将加密数据存储在链下，实现患者对医疗数据的控制权。


<details>
  <summary>Details</summary>
Motivation: 解决传统电子健康记录系统中患者数据控制权不足的问题，同时确保医疗数据的机密性、完整性和可审计性。

Method: 采用链下存储加密FHIR资源，链上记录加密承诺和患者签名的时间限制权限，使用公钥包装分发密钥，存储提供商保持诚实但好奇的状态。

Result: 权限授予的链上成本平均为78,000 gas（L1），1MB记录的端到端访问延迟为0.7-1.4秒，Layer-2部署可将gas使用量减少10-13倍。

Conclusion: 该架构为恢复患者对敏感临床数据的控制提供了一条实用路径，同时保持了所需的安防特性，并考虑了HIPAA/GDPR等监管要求。
